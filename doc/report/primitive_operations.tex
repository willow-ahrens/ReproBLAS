\section{Primitive Operations}
  ...
  \subsection{Renormalize}
    ...
    When depositing values into a $K$-fold indexed type $Y$ of index $I$, the
    assumption is made that ${Y_k}_P \in (\epsilon^{-1} 2^{a_{I + k}}, 2
    \epsilon^{-1} 2^{a_{I + k}})$ throughout the routine. To enforce this
    condition, the indexed type must be renormalized at least every $2^{p-W-2}$
    deposit operations, as will be shown in Theorem \ref{thm:renormfreq}.
    ...
    Section (lines 18-29) of Algorithm $6$ in \cite{repsum}, although it has
...
\end{figure}
  ...
  \subsection{Error Bound}
    ...
    \begin{thm}
      ...
      exists (possibly unnormalized) floating point numbers $f'_0, \ldots, f'_{n -1}$
    ...
    Which is not as tight a bound as \eqref{eq:errorapprox}, and grows linearly
