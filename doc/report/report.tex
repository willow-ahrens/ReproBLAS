  ...
  \subsection{Implementation}
    ...
    \subsubsection{Primary and Carry}
      ...
      \begin{alg}
        ...
        \label{alg:deposit}
      \end{alg}
      The last bit of $r$ is set to break ties when rounding "to nearest even" such that the amount accumulated does not depend on the size of ${Y_P}_i$ so far and the same amount ($d(r, a_{i + j}, b_{i + j})$) is always correctly deposited in each of $Y$'s primary fields assuming the value in each field satisfies previously specified constraints. Because $d(r, a_{i + j}, b_{i + j}) = 0$ for bins that are larger than the input, the top $fold$ accumulators can be computed reproducibly by shifting the accumulators towards index $0$ as necessary. \cite{repsum} contains further discussions of the reproducibility of the algorithm.
...
\section{Basic Operations}
  ...
  \subsection{Renormalize}
    ...
    First, note that $|d(x_k, a_{i + j}, b_{i + j})| \leq 2^{b_{i + j}}$, where $d(x_k, a_{i + j}, b_{i + j})$ is the amount added to ${Y_P}_j$ on iteration $j$.
  ...
  \subsection{Convert}
    ...
    These numbers may, for convenience of notation, be referred to in order as $\gamma'_0, ..., \gamma'_{2 \times fold - 1}$. We use a different symbol here to reflect the difference in indexing between the $Y$ accumulator values, $y_k$, and the above unnormalized fields of $Y$ in sorted order, $\gamma'_k$.
    We use $\gamma_k$ to represent the normalized representations of the $\gamma'_k$, and it should be noted that $\gamma_k = \gamma'_k$ as real numbers and that $\exp(\gamma_k) \leq \exp(\gamma'_k)$.
    ...
    As $|{y'_C}_0| = |\gamma'_0|$ can be bounded by $2^{e_{\max} + 1 - W + p + p - 2}$, and $\exp(\gamma'_0) < ... < \exp{\gamma'_{2 \times fold - 1}}$, we have $|\gamma'_k| \leq 2^{e_{\max} - W + p + p - 1 - k}$. The absolute value represented by an indexed type can therefore be bounded by

    $\sum\limits_{k = 0}^{2 \times fold - 1} |\gamma_k| \leq \sum\limits_{k = 0}^{2 \times fold - 1} 2^{e_{\max} - W + p + p - 1 - k} < \sum\limits_{k = 0}^{\infty} 2^{e_{\max} - W + p + p - 1 - k} = 2^{e_{\max} - W + p + p}$.
    ...
    If an intermediate type with exponent greater than $e_{\max} - W + p + p$ is not available, we must scale the $\gamma_k$ down by some factor during addition, and then scale them back up when subsequent additions can no longer effect an overflow situation.
  ...
  \subsection{Error Bound}
    ...
    \begin{thm}
      ...
      %TODO
      It is an open question as to whether or not this theorem is valid and/or applicable. However, a less formal and highly similar argument forms the basis of the proof of theorem 2 of \cite{sortsum}.
      \label{thm:sortsum2}
    \end{thm}
