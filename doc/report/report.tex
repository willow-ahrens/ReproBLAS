  ...
  \subsection{Implementation}
    ...
    \subsubsection{Primary and Carry}
      ...
      \begin{proof}
        ...
        If $r_{j + 1} = r_j - ({Z_j}_P - {Y_j}_P) = 2^{a_{I + j}}$, then since $\ulp(r_j) < 2^{a_{I + j}}$, $(r_j | 1) - r_j > 0$ and the rounding in line \ref{alg:deposit:split} must have gone towards infinity. ${Z_j}_P - {Y_j}_P = {Z_j}_P - {Y_j}_P = \roundtonearestinfty(r_j, a_{I + j} + 1)$.

        We show ${Z_j}_P - {Y_j}_P = d(x, a_{I + j}, b_{I + j})$ for all $j \in \{0, ..., fold - 1\}$ by induction.
      \end{proof}
