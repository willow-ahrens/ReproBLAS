...
\section{Notation and Background}
  ...
  We define the function $\roundfromzero(x, y)$ as $\roundfromzero(x, y) = \begin{cases}\ceil{x/y} \times y \text{ if } x \geq 0\\ \floor{x/y}\times y \text{ otherwise}\end{cases}$
...
\section{The Indexed Type}
  \subsection{Representation}
    ...
    $(e_{\min} - p + 2 + W - (e_{\max} + 1 - e_{\min} + p - 2 \mod W),$

    \indent \indent $e_{\min} - p + 2 + 2 \times W - (e_{\max} + 1 - e_{\min} + p - 2 \mod W)]$.

    Section \ref{sec:underflow} explains why input in the bin

    $(e_{\min} - p, e_{\min} - p + 2 + W + (e_{\max} + 1 - e_{\min} + p - 2 \mod W)]$
  ...
  \subsection{Implementation}
    ...
    \subsubsection{Primary and Carry}
      ...
      \begin{alg}
        ...
        \label{alg:deposit}
      \end{alg}
      The last bit of $r$ is set to break ties when rounding "to nearest even" such that the amount accumulated does not depend on the size of ${Y_P}_i$ so far and the same amount ($d(r, a_{i + j}, b_{i + j})$) is always deposited in all of $Y$'s primary fields regardless of how full they are already. (A dependency on the amount stored in $Y$'s primary fields would imply that the routine is not reproducible. Adding numbers in different orders would accumulate different amounts into the primary fields $Y_P$ when each input is added to them.) Because $d(r, a_{i + j}, b_{i + j}) = 0$ for bins that are larger than the input, the top $fold$ bins can be computed reproducibly.
 ...
 Therefore the value represented by an indexed type $Y$ of index $i$ and fold $fold$ (the sum of $Y$'s accumulators) can be expressed as in equation \ref{eq:indexedvalue}.
    ...
    \subsubsection{Overflow}
      ...
      \begin{thm}
        ...
        $a_1 = e_{\max} + 1 - 2\times W$ by definition, therefore $a_1 < e_{\max} + 1 - p$ and since all quantities are integers, $a_1 \leq e_{\max} - p$. As $a_0, a_1, ...$ is a positive decreasing sequence, $a_{i + j} \leq e_{\max} - p$ since $i + j \geq 1$.
...
\section{Basic Operations}
  ...
  \subsection{Convert}
    ...
    As each primary field ${Y_P}_j$ is an offset from $1.5 \times \epsilon^{-1} \times 2^{a_{i + j}}$, ${y_P}_j$ can be expressed exactly using an unnormalized floating point number ${y'_P}_j$ with an exponent of $a_{i + j} + p - 1$.
    ...
    $\exp({y'_C}_j) = a_{i + j} + p + p - 3$ and $\exp({y'_P}_{j - 1}) = a_{i + j - 1} + p - 1 = a_{i + j} + p - 1 + W$. Therefore $\exp({y'_C}_j) > \exp({y'_P}_{j - 1})$ using $W < p - 2$.

    $\exp({y'_C}_j) = a_{i + j} + p + p - 3$ and $\exp({y'_P}_{j - 2}) = a_{i + j - 1} + p - 1 = a_{i + j} + p - 1 + 2\times W$. Therefore $\exp({y'_C}_j) < \exp({y'_P}_{j - 2})$ using $2W > p$.

    Therefore, $\exp({y'_C}_0) > \exp({y'_C}_1) > \exp({y'_P}_0) > \exp({y'_C}_2) > \exp({y'_P}_1) > ... > \exp({y'_C}_{fold - 2}) > \exp({y'_P}_{fold - 3}) > \exp({y'_C}_{fold - 1}) > \exp({y'_P}_{fold - 2}) > \exp({y'_P}_{fold - 1})$. These numbers may, for convenience of notation, be referred to in order as $\gamma'_0, ..., \gamma'_{2 \times fold - 1}$. We use a different symbol here to reflect the difference in indexing between the accumulator values of $Y$, $y_k$, and the unnormalized fields of $Y$ in sorted order $\gamma'_k$.
    ...
    As $|{y'_C}_0| = |\gamma'_0|$ can be bounded by $2^{e_{\max} - W + p + p - 2}$, and $\exp(\gamma'_0) < ... < \exp{\gamma'_{2 \times fold - 1}}$, we have $|\gamma'_k| \leq 2^{e_{\max} - W + p + p - 2 - k}$. The absolute value represented by an indexed type can therefore be bounded by

    $\sum\limits_{k = 0}^{2 \times fold - 1} |\gamma_k| \leq \sum\limits_{k = 0}^{2 \times fold - 1} 2^{e_{\max} - W + p + p - 2 - k} < \sum\limits_{k = 0}^{\infty} 2^{e_{\max} - W + p + p - 2 - k} = 2^{e_{\max} - W + p + p - 1}$.
    ...
    \begin{alg}
      Convert indexed $Y$ with index $i$ and fold $fold$ to floating point $x$. Here, $z$ is a floating point type with precision $P$ greater than $p + \ceil{\log_2(2 \times fold)}$ and maximum exponent $E_{\max}$ greater than $e_{\max} - W + p + p - 1$
      \begin{algorithmic}
    ...
    If an intermediate type with exponent greater than $e_{\max} - W + p + p - 1$ is not available, we must scale the $\gamma'_k$ down by some factor during addition, and then scale them back up when subsequent additions can no longer effect an overflow situation.

    If the sum is to overflow, it will overflow regardless of the values of any ${y'_P}_j$ or ${y'_C}_j$ with $|{y'_P}_j| < 0.5 \times 2^{-P} \times 2^{e_{\max}}$ or $|{y'_C}_j| < 0.5 \times 2^{-P} \times 2^{e_{\max}}$. If the floating point sum of precision $P$ has exponent greater than or equal to $e_{\max}$ these numbers are not large enough to have any effect when added to the sum. If the sum has exponent less than $e_{\max}$, then additions of these numbers cannot cause the exponent of the sum to exceed $e_{\max}$ for similar reasons.

    As the maximum absolute value of the sum is bounded by $2^{e_{\max} - W + p + p - 1}$, a sufficient scaling factor is $2^{p + p - W - 1}$, bounding the maximum absolute value of the scaled sum by $2^{e_{\max}}$. When $\exp(\gamma'_k) < e_{\max} - P - 1$, we may scale the sum back up and add the remaining numbers without scaling. Notice here that no overflow occurs during addition. If an overflow is to occur, it will happen only when scaling back up.
    ...
    \begin{alg}
      ...
      \begin{algorithmic}
        \Function{ConvertIndexedToFloat}{fold, x, Y}
          \State $j \gets 1$
          \While{$j \leq 2 \times fold$ and $\exp(y'_j) \geq e_{\max} - P - 1$}
            \State $z \gets z + (y'_j / 2^{p + p - W - 1})$
            \State $j \gets j + 1$
          \EndWhile
          \State $z \gets z \times 2^{p + p - W - 1}$
  ...
  \subsection{Limits}
    ...
    As discussed in section \ref{sec:underflow}, input in the bin $(e_{\min} - p, e_{\min} - p + 2 + W + (e_{\max} - e_{\min} + p - 2 \mod W)]$ is not accumulated. For \verb|double| this amounts to the least significant expressible ? bits and for \verb|float| this amounts to the least significant expressible ? bits.

    As absolute value of individual quantities added to ${Y_P}_j$ are not in excess of $2^W b_{i + j}$, a maximum of $0.25\epsilon^{-1}2^{-W}$ elements may be deposited into ${Y_P}_j$ between renormalizations, as discussed in section \ref{sec:renormalize}. For indexed \verb|double| this number is ?, whereas for indexed \verb|float| this number is ?
