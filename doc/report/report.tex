  ...
  \subsection{Implementation}
    ...
    \subsubsection{Overflow}
      ...
      \begin{proof}
        ...
        If $I \neq 0$, then by Equation \ref{eq:b}, $|x| < 2^{b_I} < 2^{e_{\max} + 1 - W}$. By Theorem \ref{thm:overflow}, all quantities are obviously well below the overflow threshold and Theorem \ref{thm:ddepositrestricted} applies and we are done.

        If $I = 0$, more work is needed.

        We begin by showing that the amount added to ${Y_0}_P$, ($S - {Y_0}_P$ in line \ref{alg:deposit:formS}) is equal to $d(x, 0)/2^{p - W + 1}$.

        We divide into two cases based on the size of $x$.

        If $|x| < 2^{a_0}$, then regardless of whether or not there is underflow when executing line \ref{alg:deposit:scaler} (as $2^{a_0} >> 2^{e_{\min}}$), we have $|r| < 2^{a_0 - p + W - 1}$. Therefore, since ${Y_0}_P, S \in (2^{e_{\max}}, 2 \times 2^{\e_{\max}})$ in line \ref{alg:deposit:formS}, we have $\ulp({Y_0}_P) = \ulp(S)$ and because lines \ref{alg:deposit:scaler}
