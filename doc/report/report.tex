...
\section{Notation and Background}
  ...
  Assume that floating point arithmetic complies with the IEEE 754-2008 standard \cite{ieee754} in ``to nearest even'' rounding mode and that underflow occurs gradually, although the case of flush-to-zero will be considered in Section \ref{sec:underflow}.
  ...
  $\roundtonearestinfty(x, 2^k)$ rounds $x$ to the nearest multiple of $2^k$, breaking ties away from 0. A property of such rounding is shown in equation \ref{eq:round}
...
\section{The Indexed Type}
  \subsection{Binning}
    ...
    We say the bin $(a_{i_0}, b_{i_0}]$ is \textbf{higher} than the bin $(a_{i_1}, b_{i_1}]$ if $a_{i_0} < a_{i_1}$ (which is equivalent to both $b_{i_0} > b_{i_1}$ and $i_0 < i_1$).

    We say the bin $(a_{i_0}, b_{i_0}]$ is \textbf{lower} than the bin $(a_{i_1}, b_{i_1}]$ if $a_{i_0} > a_{i_1}$ (which is equivalent to both $b_{i_0} < b_{i_1}$ and $i_0 > i_1$).
    ...
    Section \ref{sec:underflow} explains why the range
    ...
    Throughout the text we will refer to the \textbf{slice} of some $x \in \F$ in the \textbf{bin} $(a_i, b_i]$. $x$ can be split into several slices, each slice corresponding to a bin $(a_i, b_i]$ and expressible as the (possibly negated) sum of a subset of $\{2^e, e \in (a_i, b_i]\}$, such that the sum of the slices provides a good approximation of $x$. Specifically, the slice of $x \in \F$ in the bin $(a_i, b_i]$ is defined as $d(x, i)$ in Equation \ref{eq:d}.
    \begin{equation}
    ...
    \begin{proof}
      ...
      In the inductive step, assume that $d(x, 0), ..., d(x, i)$ are well defined for all $x \in \F$. $d(x, i + 1)$ is expressed as a well defined expression of well defined terms.
    \end{proof}
    ...
    Theorem \ref{thm:dzero}, Theorem \ref{thm:dmiddle}, and Equation \ref{eq:d} can be combined to yield an alternate definition of $d(x, i)$ for all bins $(a_i, b_i]$ and $x \in \F$.
  ...
  \subsection{Representation}
    ...
    Each accumulator $Y_j$ is a data structure used to accumulate the slices of input in the bin $(a_{I + j}, b_{I + j}]$ where $I$ is the \textbf{index} of $Y$. The \textbf{width} of an indexed type is equal to the width of its bins, $W$. As discussed in \cite{repsum}, $W < p - 2$. As discussed in Section \ref{sec:overflow}, $2\times W > p + 1$.

    The accumulators in an indexed type correspond to contiguous bins in order of decreasing height. The index of $Y$ is defined as the least $I \in \Z$ such that $2^{b_I + 1} > \max(|x_0|, ..., |x_n|)$ (equivalently, the least $I \in \Z$ such that $b_I > \max(e_0, ..., e_n)$). If $Y$ has index $I$, then $Y_j, j \in \{0, ..., fold - 1\}$ accumulates slices of input in the bin $(a_{I + j}, b_{I + j}]$.
  ...
  \subsection{Implementation}
    ...
    \subsubsection{Primary and Carry}
      ...
      The value ${\mathcal{Y}_j}_P$ represented by data stored in the primary field ${Y_j}_P$ is an offset from $1.5\times \epsilon^{-1} \times 2^{a_{I + j}}$ (corresponding to ``$M_{[i]}$'' at the beginning of Section IV.A. in \cite{repsum}).
      \begin{equation}
...
\section{Basic Operations}
  ...
  \subsection{Error Bound}
    ...
    \begin{proof}
      ...
      If no such $m$ exists then because $S_0 = \overline{S_0}$ the computed sum is exact ($\fl(\sum \limits_{k = 0}^{n - 1} f_k) = \sum \limits_{k = 0}^{n - 1} f_k$) and we are done.

      If such an $m$ exists, then because $\exp(f_0') > ... > \exp(f_m')$, $f_0, ..., f_m \in \ulp(f_m')\Z$. Thus, $S_m \in \ulp(f_m')\Z$.

      Assume for contradiction that $|S_m| < 2 \times 2^{\exp(f_m')}$. Because $S_m \in \ulp(f_m')\Z$, this would imply that $S_m$ is representable as a floating point number with exponent $\exp(f_m')$, a contradiction as $\overline{S_m} \neq S_m$. Therefore, we have
      \begin{equation}
      ...
      If $n - 1 \leq m + 4$, we may combine Equations \ref{eq:smrelsbound} and \ref{eq:smsrelsbound} with Equation 2.4 from () to obtain
 ...
 \leq ((\frac{5\epsilon}{1 - 5 \epsilon})(\frac{2}{1 - 4 \times \sqrt\epsilon}) + (\frac{4\epsilon}{1 - 4 \epsilon})(\frac{1 + 4 \times \sqrt\epsilon}{1 - 4 \times \sqrt\epsilon}))|\sum\limits_{k = 0}^{n - 1}f_k|
      ...
      If $n - 1 > m + 4$, we may combine Equations \ref{eq:smrelsbound} and \ref{eq:smsrelsbound} with Equation 2.4 from () to obtain
 ...
 \leq ((\frac{5\epsilon}{1 - 5 \epsilon})(\frac{2}{1 - 4 \times \sqrt\epsilon}) + (\frac{4\epsilon}{1 - 4 \epsilon})(\frac{1 + 4 \times \sqrt\epsilon}{1 - 4 \times \sqrt\epsilon}))|\sum\limits_{k = 0}^{n - 1}f_k|
      ...
      Because $\exp(f_0') > ... > \exp(f_{n - 1}')$,
        ...
        \end{equation}
      By Equations \ref{eq:sbound}, \ref{eq:boundfmfivesum}, and \ref{eq:smfiveerror}.
        \begin{equation*}
        ...
        \begin{equation}
          ...
          \label{eq:minsmfour}
        \end{equation}
        As we assume $\epsilon << 1$, equation \ref{eq:minsmfour} can be simplified to
        \begin{equation}
          |\overline{S_{m + 4}}| > 2^{\exp(f_m') - 1}
          \label{eq:minsmfoursimple}
        \end{equation}

        Because $\exp(f_0') > ... > \exp(f_{n - 1}')$, we may combine equation \ref{eq:minsmfour} with \ref{eq:fmfiveexp} and have that for all $f_k$ with $k \in \{m + 5, ..., n - 1\}$,
        ...
        And therefore,
        \begin{equation}
          $\overline{S_{m + 5}} = \overline{S_{n - 1}}$
          \label{eq:smfivecontinues}
        \end{equation}
...
\end{thebibliography}
\end{document}
    Given $x_0, ..., x_{n - 1} \in \R$, the slice of $x_0, ..., x_{n - 1}$ in the bin $(a_i, b_i]$ is defined as $\sum\limits_{k = 0}^{n - 1} d(x_k, i)$.
    \begin{equation}
      d(x, i) = \begin{cases}\roundtonearestinfty(x, a_i + 1) \text{ if } 2^{b_i} > |x| \\ \roundtonearestinfty(x - \sum\limits_{k=0}^{i - 1}d(x,k), a_i + 1) \text{ if } |x| \geq 2^{b_i} \end{cases}
      \label{eq:d}
    \end{equation}
