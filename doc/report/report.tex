...
\section{The Indexed Type}
  \subsection{Representation}
    ...
    \begin{equation*}
      a_i = e_{\max} - W + 1 - i\times W, 0 \leq i < (e_{\max} - W - e_{\min} + p - 1)/W
    \end{equation*}
    ...
    We use $i < (e_{\max} - W - e_{\min} + p - 1)/W$ to ensure that $a_i > e_{\min} - p + 1$ as discussed in section \ref{sec:underflow}. This means that the highest bin is
  ...
  \subsection{Implementation}
    ...
    \subsubsection{Underflow}
      ...
      ${Y_j}_P$ is normalized and the unit in the last place of ${Y_j}_P$ is $a_{I + j} > e_{\min} - p + 1$.
        ...
        Therefore, $\ulp(r) < \ulp({Y_j}_P)/2$ and deposits on the lowest bin and all larger bins satisfy $\ulp(r) < \ulp({Y_j}_P)/2$ and are exact.
