...
\section{Notation and Background}
  ...
  \begin{equation}
    \abs{r - \roundtonearestinfty(r, e)} \leq 2^{e - 1}
    \label{eq:round}
  \end{equation}
...
\section{Binning}
  ...
  \subsection{Slices}
    ...
    \begin{proof}
      ...
      \begin{equation*}
        \abs{\roundtonearestinfty(x, a_0 + 1)} \leq \abs{x} + \abs{x - \roundtonearestinfty(x, a_0 + 1)} < 2^{a_0 + 1}
      \end{equation*}
      ...
      \begin{equation*}
        \abs{\roundtonearestinfty(x, a_{i + 1} + 1)} \leq \abs{x} + \abs{x - \roundtonearestinfty(x, a_{i + 1} + 1)} < 2^{a_{i + 1} + 1}
      \end{equation*}
    ...
    \begin{equation}
      d(x, i) = \begin{cases}0 \text{ if } \abs{x} < 2^{a_i}\\ \roundtonearestinfty(x, a_i + 1) \text{ if } 2^{a_i} \leq \abs{x} < 2^{b_i}\\\roundtonearestinfty(x - \sum\limits_{k=0}^{i - 1}d(x,k), a_i + 1) \text{ if } 2^{b_i} \leq \abs{x} \end{cases}
      \label{eq:d2}
    \end{equation}
    ...
    \begin{proof}
      ...
      \begin{equation*}
        \abs{x - \sum \limits_{k = 0}^{i}d(x, k)} = \abs{(x - \sum \limits_{k = 0}^{i - 1}d(x, k)) - d(x, i)}
      \end{equation*}
      \begin{equation*}
         = \abs{(x - \sum \limits_{k = 0}^{i - 1}d(x, k)) - \roundtonearestinfty(x - \sum \limits_{k = 0}^{i - 1}d(x, k), a_{i} + 1)} \leq 2^{a_{i}}
      \end{equation*}
    \end{proof}
    ...
    \begin{proof}
      ...
      If $i = 0$, then we have
      \begin{equation*}
        \abs{x - \sum\limits_{k=0}^{i - 1}d(x,k)} = \abs{x} < 2 \cdot 2^{e_{\max}} < 2^{b_0}
      \end{equation*}
      Otherwise, we can apply  \eqref{eq:a} and \eqref{eq:b} to Theorem \ref{thm:dround} to get
      \begin{equation*}
        \abs{x - \sum \limits_{k = 0}^{i - 1}d(x, k)} \leq 2^{a_{i - 1}} = 2^{b_i}
      \end{equation*}

      As $2^{b_i} \in 2^{a_i + 1}\Z$,  \eqref{eq:d} can be used

      \begin{equation*}
        \abs{d(x, i)} = \abs{\roundtonearestinfty(x - \sum\limits_{k=0}^{i - 1}d(x,k), a_i + 1)} \leq 2^{b_i}
      \end{equation*}
    \end{proof}
...
\section{The Indexed Type}
  \label{sec:indexed}
    ...
    \subsubsection{Gradual Underflow}
      ...
      \begin{equation}
        \abs{x - \sum\limits_{i = 0}^{i_{\max}} d(x, i)} \leq 2^{e_{\min} - 1}
    ...
    \subsubsection{Exceptions}
      ...
      \begin{equation*}
      \epsilon^{-1}  2^{e_{\min} - p + 2 + ((e_{\max} - e_{\min} + p - 1)\mod W)} > 2^{e_{\min}}
      \end{equation*}
...
\section{Primitive Operations}
  ...
  \subsection{Convert}
    ...
    \begin{equation}
      n (2^{W  (1 - K) - 1} + (2  K - 1)  \epsilon)\max\abs{x_k}
    ...
    \begin{equation}
      n 2^{W  (1 - K) - 1}\max\abs{x_k}  + 7 \epsilon \abs{\sum\limits_{k = 0}^{n - 1} x_k}
...
\end{figure}
  ...
  \subsection{Error Bound}
    ...
    \begin{proof}
      ...
      \begin{equation}
        \abs{S_m} > 2 \cdot 2^{\exp(f_m')}
        \label{eq:smbound}
      \end{equation}

      Because $\exp(f_m') > \exp(f_{m + 1}')$,
      \begin{equation}
        \abs{f_{m + 1}} < 2\cdot2^{\exp(f_m' - 1)} = 2^{\exp(f_m')}
        \label{eq:smpbound}
      \end{equation}

      Because $\exp(f_m') \geq \exp(f_{m + 2}') + \ceil{\frac{p + 1}{2}}$ and $\exp(f_0') > ... > \exp(f_{n - 1}')$,
      \begin{equation*}
        \abs{\sum \limits_{k = m + 2}^{n - 1} f_k} \leq \sum \limits_{k = m + 2}^{n - 1} \abs{f_k} < \sum \limits_{k = m + 2}^{n - 1} 2 \cdot 2^{\exp(f_k')} \leq \sum \limits_{k = m + 2}^{n - 1} 2 \cdot 2^{\exp(f_m') - \ceil{\frac{p + 1}{2}} - (m + 2 - k)}
      \end{equation*}
      \begin{equation}
      ...
      \begin{equation}
        \abs{\sum\limits_{k = m + 1}^{n - 1} f_k} \leq \sum\limits_{k = m + 1}^{n - 1} \abs{f_k} < 2^{\exp{f_m'}} + (4 \sqrt{\epsilon}) 2^{\exp(f_m')} = (1 + 4 \sqrt\epsilon )2^{\exp(f_m')}
        \label{eq:smsbound}
      \end{equation}
      ...
      \begin{equation}
        \abs{\sum \limits_{k = m + 2}^{n - 1} f_k} < (4 \sqrt{\epsilon}) 2^{\exp(f_m')} \leq \frac{4 \sqrt\epsilon}{1 - 4  \sqrt\epsilon}\abs{\sum\limits_{k = 0}^{n - 1}f_k}
        \label{eq:smpprelsbound}
      \end{equation}

      By  \eqref{eq:sbound} and \eqref{eq:smsbound},
      \begin{equation}
        \abs{\sum\limits_{k = m + 1}^{n - 1}f_k} \leq \sum\limits_{k = m + 1}^{n - 1}\abs{f_k} \leq (1 + 4  \sqrt\epsilon)2^{\exp(f_m')}\leq \frac{1 + 4  \sqrt\epsilon}{1 - 4  \sqrt\epsilon}\abs{\sum\limits_{k = 0}^{n - 1}f_k}
      ...
      \begin{equation}
        \abs{S_m} \leq \abs{\sum\limits_{k = 0}^{n - 1}f_k} + \abs{\sum\limits_{k = m + 1}^{n - 1} f_k} \leq (1 + \frac{1 + 4  \sqrt\epsilon}{1 - 4  \sqrt\epsilon})\abs{\sum\limits_{k = 0}^{n - 1}f_k} \leq \frac{2}{1 - 4  \sqrt\epsilon}\abs{\sum\limits_{k = 0}^{n - 1}f_k}
...
% TRY TO SIMPLY THE PROOF
    ...
    \begin{align*}
        ...
        & \leq \frac{4\epsilon}{1-4\epsilon} (\abs{\overline{S_m} - S_m} + \abs{S_m + f_{m+1}})
    ...
    \abs{S_{m+4} - \overline{S_{m+4}}}
        ...
        & \leq (1 + \frac{4\epsilon}{1-4\epsilon}) \abs{S_m - \overline{S_m}} + \frac{4\epsilon}{1-4\epsilon} \abs{S_{n-1}}
                + \frac{7\epsilon}{1-4\epsilon} \sum_{k=m+2}^{n-1}\abs{f_k} \\
        & \leq \frac{1}{1-4\epsilon} \epsilon \abs{S_m} + \frac{4\epsilon}{1-4\epsilon} \abs{S_{n-1}}
                + \frac{7\epsilon}{1-4\epsilon} \sum_{k=m+2}^{n-1}\abs{f_k} \\
        & \leq \frac{\epsilon}{1-4\epsilon} \left(\abs{S_m} + 4 \abs{S_{n-1}}
                + 7 \sum_{k=m+2}^{n-1}\abs{f_k}\right). 
    \end{align*}
    ...
    \abs{S_{m+4} - \overline{S_{m+4}}}
        ...
        & \leq \frac{\epsilon}{1-4\epsilon} \frac{6+12\sqrt{\epsilon}}{1-4\sqrt{\epsilon}} \abs{S_{n-1}}
            = \frac{6\epsilon }{(1-2\sqrt{\epsilon})(1-4\sqrt{\epsilon})} \abs{S_{n-1}} \nonumber \\
    ...
    \end{align}
      ...
      Because $\exp(f_0') > ... > \exp(f_{n - 1}')$, \eqref{eq:fmfiveexp} yields
        \begin{equation}
          \abs{\sum\limits_{k = m + 5}^{n - 1} f_k} \leq \sum\limits_{k = m + 5}^{n - 1} \abs{f_k} < \sum\limits_{k = m + 5}^{n - 1} 2 \cdot 2^{\exp(f_m') - p - 2 - (k - (m + 5))} < \sum\limits_{k = 0}^{\infty} 2^{\exp(f_m') - p - 1 - k} = \epsilon 2^{\exp(f_m')}
      ...
      By  \eqref{eq:sbound}, \eqref{eq:smfiveerror}, and \eqref{eq:boundfmfivesum}
        \begin{equation*}
          \abs{\overline{S_{m + 4}}} \geq \abs{S_{m + 4}} - \abs{S_{m + 4} - \overline{S_m + 4}} \geq (\abs{\sum\limits_{k = 0}^{n - 1} f_k} - \abs{\sum\limits_{k = m + 5}^{n - 1}f_k}) - \abs{S_{m + 4} - \overline{S_m + 4}}
        \end{equation*}
        \begin{equation*}
           = (\abs{\sum\limits_{k = 0}^{n - 1} f_k} - \abs{S_{m + 4} - \overline{S_m + 4}}) - \abs{\sum\limits_{k = m + 5}^{n - 1}f_k}
        \end{equation*}
        \begin{equation*}
           > (1 - \frac{6\epsilon}{1 - 6\sqrt\epsilon})\abs{\sum\limits_{k = 0}^{n - 1} f_k} - \abs{\sum\limits_{k = m + 5}^{n - 1}f_k}
        \end{equation*}
        \begin{equation*}
        ...
        \begin{equation}
          \abs{\overline{S_{m + 4}}} > 2^{\exp(f_m') - 1}
        ...
        \begin{equation}
          \abs{f_k} < 2 \cdot 2^{\exp(f_k')} \leq 2 \cdot 2^{\exp(f_m) - p - 2} = \epsilon \cdot 2^{\exp(f_m') - 1}
          \label{eq:maxfmfive}
        \end{equation}
    ...
    \begin{equation}
      ...
      \abs{T - \mathcal{Y}} < n  2^{W  (1 - K) - 1}\max\abs{x_k}
    \end{equation}
    ...
    \begin{equation}
      \label{eq:repbound}
      \abs{T - \mathcal{Y}} < n \cdot \max(2^{W  (1 - K) - 1} \max\abs{x_k}, 2^{e_{\min} - 1})
    \end{equation}
    ...
    \begin{equation*}
      \abs{\mathcal{Y}} \leq \abs{T} + \abs{T - \mathcal{Y}} < n \cdot \max(2^{W  (1 - K) - 1}  \max\abs{x_k}, 2^{e_{\min} - 1}) + \abs{T}
    \end{equation*}
    ...
    \begin{equation*}
      < n \cdot \max(2^{W  (1 - K) - 1}  \max\abs{x_k}, 2^{e_{\min} - 1}) + \frac{7\epsilon}{1 - 6\sqrt\epsilon} \abs{\mathcal{Y}}
    \end{equation*}

    \begin{equation*}
      < n \cdot \max(2^{W  (1 - K) - 1}  \max\abs{x_k}, 2^{e_{\min} - 1})
    \end{equation*}
    \begin{equation*}
      + \frac{7\epsilon}{1 - 6\sqrt\epsilon} (n \cdot \max(2^{W  (1 - K) - 1}  \max\abs{x_k}, 2^{e_{\min} - 1}) + \abs{T})
    \end{equation*}

    \begin{equation}
      < (1 + \frac{7\epsilon}{1 - 6\sqrt\epsilon}) (n \cdot \max(2^{W (1 - K) - 1} \max\abs{x_k}, 2^{e_{\min} - 1})) + \frac{7\epsilon}{1 - 6\sqrt\epsilon} \abs{T}
      \label{eq:error}
    \end{equation}
    ...
    \begin{equation*}
      \abs{T - \overline{\mathcal{Y}}} < (1 + \frac{7\epsilon}{1 - 6\sqrt\epsilon}) (n \cdot \max(2^{W (1 - K) - 1} \max\abs{x_k}, 2^{e_{\min} - 1})) + \frac{7\epsilon}{1 - 6\sqrt\epsilon}\abs{T}
    \end{equation*}

    \begin{equation}
    \approx n 2^{W  (1 - K) - 1} \max\abs{x_k} + 7  \epsilon \abs{T}
    ...
    \begin{equation*}
      \abs{\mathcal{Y}} < (\frac{1}{1 - \frac{7\epsilon}{1 - 6\sqrt\epsilon}})  \abs{\overline{\mathcal{Y}}}
    \end{equation*}
    ...
    \begin{equation*}
      < n \cdot \max(2^{W  (1 - K) - 1}  \max\abs{x_k}, 2^{e_{\min} - 1}) + \frac{7\epsilon}{1 - 6\sqrt\epsilon}\frac{1 - 6 \sqrt\epsilon}{1 - 6 \sqrt \epsilon - 7\epsilon}\abs{\overline{\mathcal{Y}}}
    \end{equation*}

    \begin{equation}
      < n \cdot \max(2^{W  (1 - K) - 1}  \max\abs{x_k}, 2^{e_{\min} - 1}) + \frac{7\epsilon}{1 - 6 \sqrt \epsilon - 7\epsilon}\abs{\overline{\mathcal{Y}}}
      \label{eq:error2}
    \end{equation}
    ...
    \begin{equation*}
      \abs{T - \overline{\mathcal{Y}}} < n \cdot \max(2^{W  (1 - K) - 1}  \max\abs{x_k}, 2^{e_{\min} - 1}) + \frac{7\epsilon}{1 - 6 \sqrt \epsilon - 7\epsilon}  \abs{\overline{\mathcal{Y}}}
    \end{equation*}

    \begin{equation}
      \approx n  2^{W  (1 - K) - 1} \max\abs{x_k}+ 7  \epsilon  \abs{\overline{\mathcal{Y}}}
    ...
    \begin{equation*}
    n \cdot \max(2^{W  (1 - K) - 1}  \max\abs{x_k}, 2^{e_{\min} - 1}) + (\frac{(2  K - 1)  \epsilon}{1 - (2  K - 1)  \epsilon})  \sum\limits_0^{2  K - 1}\abs{\gamma_k}
    \end{equation*}

    \begin{equation*}
    \leq n \cdot \max(2^{W  (1 - K) - 1}  \max\abs{x_k}, 2^{e_{\min} - 1}) + (\frac{(2  K - 1)  \epsilon}{1 - (2  K - 1)  \epsilon})  n \cdot \max\abs{x_k}
    \end{equation*}

    \begin{equation}
    \approx n \cdot \max\abs{x_k} (2^{W  (1 - K) - 1} + (2  K - 1)  \epsilon)
