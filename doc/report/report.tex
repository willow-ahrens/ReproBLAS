...
\DeclarePairedDelimiter{\prs*}{\lparen}{\rparen}
\DeclarePairedDelimiter{\abs}{\lvert}{\rvert}
\DeclarePairedDelimiter{\bin}{\lparen}{\rbrack}
...
\section{Notation and Background}
  ...
  \begin{equation}
    \roundtonearestinfty\prs*{r, e} = \begin{cases}\floor*{r/2^e + 1/2} 2^e \text{ if } r \geq 0\\ \\ \ceil*{r/2^e - 1/2} 2^e \text{ otherwise}\end{cases}
  \end{equation}

  $\roundtonearestinfty\prs*{r, e}$ rounds $r$ to the nearest multiple of $2^e$, breaking ties away from 0. A property of such rounding is shown in  \eqref{eq:round}

  \begin{equation}
    \abs*{r - \roundtonearestinfty\prs*{r, e}} \leq 2^{e - 1}
    \label{eq:round}
  \end{equation}
  ...
  For all $f_0, f_1 \in \F$, $\fl(f_0 \text{ op } f_1)$ denotes the evaluated result of the expression $(f_0 \text{ op } f_1)$ in floating point arithmetic. If $(f_0 \text{ op } f_1)$ is representable, then $\fl(f_0 \text{ op } f_1) = (f_0 \text{ op } f_1)$.
  ...
  The unit in the last place of $f \in \F$, $\ulp(f)$, is the spacing between two consecutive floating point numbers of the same exponent as $f$. If $f$ is normalized, $\ulp(f) = 2^{\exp(f) - p + 1} = 2  \epsilon  2^{\exp(f)}$.
...
\section{Binning}
  ...
  \subsection{Bins}
    ...
    \begin{equation}
      i_{\max} = \floor*{\prs*{e_{\max} - e_{\min} + p - 1}/W} - 1
      \label{eq:imax}
    \end{equation}

    \begin{equation}
      a_i = e_{\max} + 1 - \prs*{i + 1}W,\; 0 \leq i \leq i_{\max}
      \label{eq:a}
    \end{equation}
    ...
    \begin{equation}
      \bin*{e_{\min} - p + 2 + \prs*{\prs*{e_{\max} - e_{\min} + p - 1}\mod W},
      e_{\min} - p + 2 + W + \prs*{\prs*{e_{\max} - e_{\min} + p - 1}\mod W}}
      \label{eq:binmin}
    \end{equation}
    ...
    \begin{equation*}
    \bin*{e_{\min} - p, e_{\min} - p + 2 + \prs*{\prs*{e_{\max} - e_{\min} + p - 1} \mod W}}
    \end{equation*}
  ...
  \subsection{Slices}
    ...
    \begin{equation}
      d\prs*{x, i} = \roundtonearestinfty\prs*{x - \sum\limits_{k=0}^{i - 1}d\prs*{x,k}, a_i + 1}
      \label{eq:d}
    \end{equation}
    ...
    \begin{proof}
      ...
      \begin{equation*}
        \abs*{\roundtonearestinfty\prs*{x, a_0 + 1}} \leq \abs*{x} + \abs*{x - \roundtonearestinfty\prs*{x, a_0 + 1}} < 2^{a_0 + 1}
      \end{equation*}
      Since $\roundtonearestinfty(x, a_0 + 1) \in 2^{a_0 + 1}\Z$, we then have
      \begin{equation*}
        d\prs*{x, 0} = \roundtonearestinfty\prs*{x, a_0 + 1} = 0
      \end{equation*}

      In the inductive step, we have $|x| < 2^{a_{i + 1}} \leq ... \leq 2^{a_0}$ by  \eqref{eq:a} and by induction $d(x, i)= ... = d(x, 0) = 0$. Thus,
      \begin{equation*}
        d\prs*{x, i + 1} = \roundtonearestinfty\prs*{x - \sum\limits_{k = 0}^{i}d\prs*{x, k}, a_{i + 1} + 1} = \roundtonearestinfty\prs*{x, a_{i+1} + 1}
      \end{equation*}
      Again by the triangle inequality and  \eqref{eq:round},
      \begin{equation*}
        \abs*{\roundtonearestinfty\prs*{x, a_{i + 1} + 1}} \leq \abs*{x} + \abs*{x - \roundtonearestinfty\prs*{x, a_{i + 1} + 1}} < 2^{a_{i + 1} + 1}
      \end{equation*}
      Since $\roundtonearestinfty(x, a_{i + 1} + 1) \in 2^{a_{i + 1} + 1}\Z$, we then have
      \begin{equation*}
        d\prs*{x, i + 1} = \roundtonearestinfty\prs*{x, a_{i + 1} + 1} = 0
      \end{equation*}
    \end{proof}
    ...
    \begin{proof}
      ...
      By  \eqref{eq:a} and \eqref{eq:b}, $|x| < 2^{b_i} = 2^{a_{i - 1}} \leq  ... \leq 2^{a_0}$. Therefore Lemma \ref{lem:dzero} implies $d(x, 0) = ... = d(x, i - 1) = 0$ and we have \begin{equation*}
        d\prs*{x, i} = \roundtonearestinfty\prs*{x - \sum\limits_{k = 0}^{i - 1}d\prs*{x, k}, a_{i} + 1} = \roundtonearestinfty\prs*{x, a_{i} + 1}
      \end{equation*}
    \end{proof}
    ...
    \begin{equation}
      d\prs*{x, i} = \begin{cases}0 \text{ if } \abs*{x} < 2^{a_i}\\ \roundtonearestinfty\prs*{x, a_i + 1} \text{ if } 2^{a_i} \leq \abs*{x} < 2^{b_i}\\\roundtonearestinfty\prs*{x - \sum\limits_{k=0}^{i - 1}d\prs*{x,k}, a_i + 1} \text{ if } 2^{b_i} \leq \abs*{x} \end{cases}
      \label{eq:d2}
    \end{equation}
    ...
    \begin{proof}
      ...
      \begin{equation*}
        \abs*{x - \sum \limits_{k = 0}^{i}d\prs*{x, k}} = \abs*{\prs*{x - \sum \limits_{k = 0}^{i - 1}d\prs*{x, k}} - d\prs*{x, i}}
      \end{equation*}
      \begin{equation*}
         = \abs*{\prs*{x - \sum \limits_{k = 0}^{i - 1}d\prs*{x, k}} - \roundtonearestinfty\prs*{x - \sum \limits_{k = 0}^{i - 1}d\prs*{x, k}, a_{i} + 1}} \leq 2^{a_{i}}
      \end{equation*}
    \end{proof}
    ...
    \begin{proof}
      ...
      If $i = 0$, then we have
      \begin{equation*}
        \abs*{x - \sum\limits_{k=0}^{i - 1}d\prs*{x,k}} = \abs*{x} < 2 \cdot 2^{e_{\max}} < 2^{b_0}
      \end{equation*}
      Otherwise, we can apply  \eqref{eq:a} and \eqref{eq:b} to Theorem \ref{thm:dround} to get
      \begin{equation*}
        \abs*{x - \sum \limits_{k = 0}^{i - 1}d\prs*{x, k}} \leq 2^{a_{i - 1}} = 2^{b_i}
      \end{equation*}

      As $2^{b_i} \in 2^{a_i + 1}\Z$,  \eqref{eq:d} can be used

      \begin{equation*}
        \abs*{d\prs*{x, i}} = \abs*{\roundtonearestinfty\prs*{x - \sum\limits_{k=0}^{i - 1}d\prs*{x,k}, a_i + 1}} \leq 2^{b_i}
      \end{equation*}
    \end{proof}
...
\section{The Indexed Type}
  \label{sec:indexed}
    ...
    \subsection{Primary and Carry}
      ...
      \begin{equation}
        {\mathcal{Y}_j}_C = \prs*{0.25\epsilon^{-1}2^{a_{I + j}}}{Y_j}_C
        \label{eq:car}
      \end{equation}
      ...
      \begin{equation}
        \mathcal{Y}_j = {\mathcal{Y}_j}_P + {\mathcal{Y}_j}_C = \prs*{{Y_j}_P - 1.5 \epsilon^{-1}2^{a_{I + j}}} + \prs*{0.25\epsilon^{-1}2^{a_{I + j}}}{Y_j}_C
        \label{eq:acc}
      \end{equation}
      ...
      \begin{equation}
        \mathcal{Y} = \sum\limits_{j = 0}^{K - 1} \mathcal{Y}_j = \sum\limits_{j = 0}^{K - 1} \prs*{\prs*{{Y_j}_P - 1.5 \epsilon^{-1}2^{a_{I + j}}} + \prs*{0.25\epsilon^{-1}2^{a_{I + j}}}{Y_j}_C}
    ...
    \end{samepage}
      ...
      \begin{proof}
        ...
        \begin{equation*}
          \abs*{{Y_j}_P} < 2 \epsilon^{-1} 2^{a_{I + j}} \leq 2^{1 + p} 2^{e_{\max} - 1 - p} = 2^{e_{\max}}
        \end{equation*}
      \end{proof}
      ...
      \begin{equation}
        {\mathcal{Y}_0}_P = 2^{p - W + 1}\prs*{{Y_0}_P - 1.5\cdot2^{e_{\max}}}
        \label{eq:pri0}
      \end{equation}
    \subsubsection{Gradual Underflow}
      ...
      \begin{proof}
        The least bin is
        \begin{equation*}
        \bin*{e_{\min} - p + 2 + \prs*{\prs*{e_{\max} - e_{\min} + p - 1 } \mod W}, e_{\min} - p + 2 + W + \prs*{\prs*{e_{\max} - e_{\min} + p - 1}\mod W}}
        \end{equation*}

        Because ${Y_j}_P \in (\epsilon^{-1} 2^{a_{I + j}}, 2 \epsilon^{-1} 2^{a_{I + j}})$ and
        \begin{equation*}
        a_{I + j} \geq e_{\min} - p + 2 + \prs*{\prs*{e_{\max} - e_{\min} + p - 1} \mod W} \geq e_{\min} - p + 2
        \end{equation*}
      ...
      \begin{equation*}
        a_{i_{\max}} = e_{\min} - p + 2 + \prs*{\prs*{e_{\max} - e_{\min} + p - 1 } \mod W} \leq {e_{\min} - 1}
      \end{equation*}

      As a consequence, we can use Theorem \ref{thm:dround} to say that for any $x \in \F$,
      \begin{equation}
        \abs*{x - \sum\limits_{i = 0}^{i_{\max}} d\prs*{x, i}} \leq 2^{e_{\min} - 1}
    ...
    \subsubsection{Exceptions}
      ...
      \bin*{e_{\min} - p + 2 + \prs*{\prs*{e_{\max} - e_{\min} + p - 1 } \mod W}, e_{\min} - p + 2 + W + \prs*{\prs*{e_{\max} - e_{\min} + p - 1}\mod W}}
      \end{equation*}
      and since the ${Y_j}_P$ are kept within the range $(\epsilon^{-1}  2^{a_{I + j}}, 2  \epsilon^{-1}  2^{a_{I + j}})$ (where $I$ is the index), the least possible ${Y_j}_P$ is

      \begin{equation*}
      \epsilon^{-1}  2^{e_{\min} - p + 2 + \prs*{\prs*{e_{\max} - e_{\min} + p - 1}\mod W}} > 2^{e_{\min}}
      \end{equation*}
...
\section{Primitive Operations}
  ...
  \subsection{Deposit}
    ...
    \end{samepage}
      ...
      \begin{proof}
        ...
        As ${Y_j}_P, S_j \in (\epsilon^{-1}  2^{a_{I + j}}, 2  \epsilon^{-1}  2^{a_{I + j}})$, we have $\ulp(S_j) = \ulp({Y_j}_P)$ for all $j$.
        ...
        \begin{equation*}
          S_{j + 1} - {Y_{j + 1}}_P = \roundtonearestinfty\prs*{r_{j + 1}, a_{I + j + 1} + 1} = \roundtonearestinfty\prs*{x - \sum\limits_{i = 0}^{I + j}d\prs*{x, i}, a_{I + j + 1} + 1} = d\prs*{x, I + j + 1}
        \end{equation*}

        And by Theorem 3 of \cite{repsum},
        \begin{equation*}
          r_{j + 2} = r_{j + 1} - \prs*{S_{j + 1} - {Y_{j + 1}}_P} = \prs*{x - \sum\limits_{i = 0}^{I + j}d\prs*{x, i}} - d\prs*{x, I + j + 1} = x - \sum\limits_{i = 0}^{I + j + 1}d\prs*{x, i}
        \end{equation*}
      \end{proof}
    ...
    \end{samepage}
      ...
      \begin{proof}
        ...
        Since ${Y_0}_P, S \in (2^{e_{\max}}, 2 \cdot 2^{e_{\max}})$, we have $\ulp({Y_0}_P) = \ulp(S)$ in line \ref{alg:deposit:formS}. As $|r| < 2^{e_{\max} - p + W}$ and $W < p - 2$, we have $\ulp(r) < \frac{1}{2}\ulp({Y_0}_P)$in line \ref{alg:deposit:formS}.
        ...
        If $x > 0$, we have
        \begin{equation*}
          x \geq x - d\prs*{x, 0}/2 \geq x - d\prs*{x, 0} \geq -x
        \end{equation*}
         If $x < 0$, we have
        \begin{equation*}
          -x \geq x - d\prs*{x, 0} \geq x - d\prs*{x, 0}/2 \geq x
        \end{equation*}
  ...
  \subsection{Renormalize}
    ...
    \begin{equation*}
    |\sum \limits_{n = 0}^{k - 1} d\prs*{x_k, I + j}| \leq n  2^{b_{I + j}} = n  2^{W}  2^{a_{I + j}}
    \end{equation*}

    If $n \leq 2^{p - W - 2}$, then after the $n^{th}$ deposit

    \begin{equation*}
    {Y_j}_P \in [\prs*{1.5  \epsilon^{-1} - n  2^W} 2^{a_{I + j}}, \prs*{1.75  \epsilon^{-1} + n  2^W} 2^{a_{I + j}}]
    \end{equation*}
    ...
    \begin{equation}
      0.25\epsilon^{-1}2^{-W}  \prs*{\epsilon^{-1} - 1} \approx 2^{2  p - W - 2}
      \label{eq:totalfreq}
    \end{equation}
  ...
  \subsection{Convert}
    ...
    \begin{equation}
      n \prs*{2^{W  \prs*{1 - K} - 1} + \prs*{2  K - 1}  \epsilon}\max\abs*{x_k}
    ...
    \begin{equation}
      n 2^{W  \prs*{1 - K} - 1}\max\abs*{x_k}  + 7 \epsilon \abs*{\sum\limits_{k = 0}^{n - 1} x_k}
    ...
    \begin{equation}
      n  \epsilon  \sum\limits_{k = 0}^{n - 1}\abs*{x_k} \leq n^2  \epsilon  \max\abs*{x_k}
...
\end{figure}
    ...
    Note that
    \begin{equation*}
      \exp\prs*{{\mathcal{Y}'_j}_C} = a_{I + j} + 2  p - 3
    \end{equation*}

    and

    \begin{equation*}
      \exp\prs*{{\mathcal{Y}'_{j - 1}}_P} = a_{I + j - 1} + p - 1 = a_{I + j} + W + p - 1
    \end{equation*}
    ...
    \begin{equation*}
      \exp\prs*{{\mathcal{Y}'_{j - 2}}_P} = a_{I + j - 1} + p - 1 = a_{I + j} + 2 W + p - 1
    \end{equation*}
    ...
    \begin{equation*}
    \exp\prs*{{\mathcal{Y}'_0}_C} > \exp\prs*{{\mathcal{Y}'_1}_C} > \exp\prs*{{\mathcal{Y}'_0}_P} > \exp\prs*{{\mathcal{Y}'_2}_C} > \exp\prs*{{\mathcal{Y}'_1}_P} > ...
    \end{equation*}
    \begin{equation*}
    ...> \exp\prs*{{\mathcal{Y}'_j}_C} > \exp\prs*{{\mathcal{Y}'_{j - 1}}_P} > \exp\prs*{{\mathcal{Y}'_{j + 1}}_C} > \exp\prs*{{\mathcal{Y}'_j}_P} > ...
    \end{equation*}
    \begin{equation*}
    ...> \exp\prs*{{\mathcal{Y}'_{K - 2}}_C} > \exp\prs*{{\mathcal{Y}'_{K - 3}}_P} > \exp\prs*{{\mathcal{Y}'_{K - 1}}_C} > \exp\prs*{{\mathcal{Y}'_{K - 2}}_P} > \exp\prs*{{\mathcal{Y}'_{K - 1}}_P}
    \end{equation*}
    ...
    \begin{equation}
      \exp\prs*{\gamma_0'} > ... > \exp\prs*{\gamma_{2  K - 1}'}
    ...
    \begin{equation}
      \exp\prs*{\gamma_k'} \geq \exp\prs*{\gamma_{k + 2}'} + W \geq \exp\prs*{\gamma_{k + 2}'} + \ceil*{\frac{p + 1}{2}}
    ...
    \begin{equation}
      ...
      \sum\limits_{k = 0}^{2  K - 1} \abs*{\gamma_k} < \sum\limits_{k = 0}^{2  K - 1} 2^{e_{\max} - W + 2  p - 1 - k} < \sum\limits_{k = 0}^{\infty} 2^{e_{\max} - W + 2  p - 1 - k} = 2^{e_{\max} - W + 2  p}
    \end{equation}
  ...
  \subsection{Error Bound}
    ...
    \begin{thm}
      ...
      Then we have
      \begin{equation*}
        \abs*{S_{n - 1} - \overline{S_{n - 1}}} < \frac{7\epsilon}{1 - 6\sqrt\epsilon}\abs*{\sum\limits_{k = 0}^{n - 1}f_k} \approx 7 \epsilon \abs*{\sum\limits_{k = 0}^{n - 1}f_k}
      \end{equation*}
    \end{thm}
    \end{samepage}
    ...
    \begin{proof}
      ...
      \begin{equation}
        \abs*{S_m} > 2 \cdot 2^{\exp\prs*{f_m'}}
        \label{eq:smbound}
      \end{equation}

      Because $\exp(f_m') > \exp(f_{m + 1}')$,
      \begin{equation}
        \abs*{f_{m + 1}} < 2\cdot2^{\exp\prs*{f_m' - 1}} = 2^{\exp\prs*{f_m'}}
        \label{eq:smpbound}
      \end{equation}

      Because $\exp(f_m') \geq \exp(f_{m + 2}') + \ceil{\frac{p + 1}{2}}$ and $\exp(f_0') > ... > \exp(f_{n - 1}')$,
      \begin{equation*}
        \abs*{\sum \limits_{k = m + 2}^{n - 1} f_k} \leq \sum \limits_{k = m + 2}^{n - 1} \abs*{f_k} < \sum \limits_{k = m + 2}^{n - 1} 2 \cdot 2^{\exp\prs*{f_k'}} \leq \sum \limits_{k = m + 2}^{n - 1} 2 \cdot 2^{\exp\prs*{f_m'} - \ceil*{\frac{p + 1}{2}} - \prs*{m + 2 - k}}
      \end{equation*}
      \begin{equation}
        < \sum \limits_{k = 0}^{\infty} \prs*{2 \sqrt{\epsilon}}2^{\exp\prs*{f_m'} - k} = \prs*{4\sqrt\epsilon}2^{\exp\prs*{f_m'}}
        \label{eq:smppbound}
      \end{equation}
      ...
      \begin{equation}
        \abs*{\sum\limits_{k = m + 1}^{n - 1} f_k} \leq \sum\limits_{k = m + 1}^{n - 1} \abs*{f_k} < 2^{\exp{f_m'}} + \prs*{4 \sqrt{\epsilon}} 2^{\exp\prs*{f_m'}} = \prs*{1 + 4 \sqrt\epsilon }2^{\exp\prs*{f_m'}}
        \label{eq:smsbound}
      \end{equation}
      ...
      \begin{align}
        \abs*{S_{n-1}} & = \abs*{\sum\limits_{k = 0}^{n - 1} f_k} \geq \abs*{\sum\limits_{k = 0}^{m} f_k} - \abs*{\sum\limits_{k = m + 1}^{n - 1} f_k} = \abs*{S_m} - \abs*{\sum\limits_{k = m + 1}^{n - 1} f_k} \nonumber \\
        & \geq 2 \cdot 2^{\exp\prs*{f_{m}'}} - \prs*{1 + 4 \sqrt\epsilon} 2^{\exp\prs*{f_m'}} = \prs*{1 - 4 \sqrt\epsilon} 2^{\exp\prs*{f_m'}}
        \label{eq:sbound}
      \end{align}

      By  \eqref{eq:sbound} and \eqref{eq:smppbound},
      \begin{equation}
        \abs*{\sum \limits_{k = m + 2}^{n - 1} f_k} < \prs*{4 \sqrt{\epsilon}} 2^{\exp\prs*{f_m'}} \leq \frac{4 \sqrt\epsilon}{1 - 4  \sqrt\epsilon}\abs*{\sum\limits_{k = 0}^{n - 1}f_k}
        \label{eq:smpprelsbound}
      \end{equation}

      By  \eqref{eq:sbound} and \eqref{eq:smsbound},
      \begin{equation}
        \abs*{\sum\limits_{k = m + 1}^{n - 1}f_k} \leq \sum\limits_{k = m + 1}^{n - 1}\abs*{f_k} \leq \prs*{1 + 4  \sqrt\epsilon}2^{\exp\prs*{f_m'}}\leq \frac{1 + 4  \sqrt\epsilon}{1 - 4  \sqrt\epsilon}\abs*{\sum\limits_{k = 0}^{n - 1}f_k}
      ...
      \begin{equation}
        \abs*{S_m} \leq \abs*{\sum\limits_{k = 0}^{n - 1}f_k} + \abs*{\sum\limits_{k = m + 1}^{n - 1} f_k} \leq \prs*{1 + \frac{1 + 4  \sqrt\epsilon}{1 - 4  \sqrt\epsilon}}\abs*{\sum\limits_{k = 0}^{n - 1}f_k} \leq \frac{2}{1 - 4  \sqrt\epsilon}\abs*{\sum\limits_{k = 0}^{n - 1}f_k}
...
% TRY TO SIMPLY THE PROOF
    ...
    \begin{align*}
        \abs*{\overline{S_m} + \sum_{k=m+1}^{m+4}f_k - \overline{S_{m+4}}}
        & \leq \frac{4\epsilon}{1-4\epsilon} \abs*{\overline{S_m} + f_{m+1}} + \frac{3\epsilon}{1-3\epsilon} \sum_{k=m+2}^{m+4}\abs*{f_k} \\
        & \leq \frac{4\epsilon}{1-4\epsilon} \prs*{\abs*{\overline{S_m} - S_m} + \abs*{S_m + f_{m+1}}}
            + \frac{3\epsilon}{1-3\epsilon} \sum_{k=m+2}^{n-1}\abs*{f_k}.
    \end{align*}

    Since $S_{n-1} = S_m + f_{m+1} + \sum_{k=m+2}^{n-1} f_k$, we have
    \[
        \abs*{S_m + f_{m+1}}
        = \abs*{S_{n-1} - \sum_{k=m+2}^{n-1}f_k}
        \leq \abs*{S_{n-1}} + \sum_{k=m+2}^{n-1} \abs*{f_k}
    \]
    Therefore
    \[
        \abs*{\overline{S_m} + \sum_{k=m+1}^{m+4}f_k - \overline{S_{m+4}}}
        \leq \frac{4\epsilon}{1-4\epsilon} \abs*{S_m - \overline{S_m}}
        + \frac{4\epsilon}{1-4\epsilon} \abs*{S_{n-1}}
        + \frac{7\epsilon}{1-4\epsilon} \sum_{k=m+2}^{n-1}\abs*{f_k}.
    \]

    Using the Triangle Inequality we have
    \begin{align*}
    \abs*{S_{m+4} - \overline{S_{m+4}}}
        & = \abs*{S_m + \sum_{k=m+1}^{m+4}f_k - \overline{S_{m+4}}}
        \leq \abs*{S_m - \overline{S_m}} + \abs*{\overline{S_m} + \sum_{k=m+1}^{m+4}f_k - \overline{S_{m+4}}} \\
        & \leq \prs*{1 + \frac{4\epsilon}{1-4\epsilon}} \abs*{S_m - \overline{S_m}} + \frac{4\epsilon}{1-4\epsilon} \abs*{S_{n-1}}
                + \frac{7\epsilon}{1-4\epsilon} \sum_{k=m+2}^{n-1}\abs*{f_k} \\
        & \leq \frac{1}{1-4\epsilon} \epsilon \abs*{S_m} + \frac{4\epsilon}{1-4\epsilon} \abs*{S_{n-1}}
                + \frac{7\epsilon}{1-4\epsilon} \sum_{k=m+2}^{n-1}\abs*{f_k} \\
        & \leq \frac{\epsilon}{1-4\epsilon} \prs*{\abs*{S_m} + 4 \abs*{S_{n-1}}
                + 7 \sum_{k=m+2}^{n-1}\abs*{f_k}}.
    \end{align*}
    ...
    \abs*{S_{m+4} - \overline{S_{m+4}}}
        & \leq \frac{\epsilon}{1-4\epsilon}
            \prs*{
                \frac{2}{1-4\sqrt{\epsilon}} \abs*{S_{n-1}}
                + 4 \abs*{S_{n-1}}
                + 7 \frac{4\sqrt{\epsilon}}{1-4\sqrt{\epsilon}} \abs*{S_{n-1}}
            } \nonumber \\
        & \leq \frac{\epsilon}{1-4\epsilon} \frac{6+12\sqrt{\epsilon}}{1-4\sqrt{\epsilon}} \abs*{S_{n-1}}
            = \frac{6\epsilon }{\prs*{1-2\sqrt{\epsilon}}\prs*{1-4\sqrt{\epsilon}}} \abs*{S_{n-1}} \nonumber \\
        & < \frac{6\epsilon}{1-6\sqrt{\epsilon}} \abs*{S_{n-1}}
        \label{eq:smfiveerror}
    \end{align}

      Notice that
        \begin{equation*}
          \exp\prs*{f_m'} \geq \exp\prs*{f_{m + 2}'} + \ceil*{\frac{p+ 1}{2}} \geq \exp\prs*{f_{m + 4}'} + 2  \ceil*{\frac{p + 1}{2}} > \exp\prs*{f_{m + 5}'}+ 2  \ceil*{\frac{p+ 1}{2}}
        \end{equation*}
      Therefore,
        \begin{equation}
          \exp\prs*{f_m'} \geq \exp\prs*{f_{m + 5}'} + p + 2
          \label{eq:fmfiveexp}
        \end{equation}

      Because $\exp(f_0') > ... > \exp(f_{n - 1}')$, \eqref{eq:fmfiveexp} yields
        \begin{equation}
          \abs*{\sum\limits_{k = m + 5}^{n - 1} f_k} \leq \sum\limits_{k = m + 5}^{n - 1} \abs*{f_k} < \sum\limits_{k = m + 5}^{n - 1} 2 \cdot 2^{\exp\prs*{f_m'} - p - 2 - \prs*{k - \prs*{m + 5}}} < \sum\limits_{k = 0}^{\infty} 2^{\exp\prs*{f_m'} - p - 1 - k} = \epsilon 2^{\exp\prs*{f_m'}}
          \label{eq:boundfmfivesum}
        \end{equation}

      Using  \eqref{eq:sbound} and \eqref{eq:boundfmfivesum},
        \begin{equation}
          \abs*{\sum\limits_{k = m + 5}^{n - 1} f_k} \leq \sum\limits_{k = m + 5}^{n - 1} \abs*{f_k} < \frac{\epsilon}{1 - 4  \sqrt\epsilon}\abs*{\sum\limits_{k = 0}^{n - 1} f_k}
          \label{eq:relsboundfmfivesum}
        \end{equation}

      By  \eqref{eq:sbound}, \eqref{eq:smfiveerror}, and \eqref{eq:boundfmfivesum}
        \begin{equation*}
          \abs*{\overline{S_{m + 4}}} \geq \abs*{S_{m + 4}} - \abs*{S_{m + 4} - \overline{S_m + 4}} \geq \prs*{\abs*{\sum\limits_{k = 0}^{n - 1} f_k} - \abs*{\sum\limits_{k = m + 5}^{n - 1}f_k}} - \abs*{S_{m + 4} - \overline{S_m + 4}}
        \end{equation*}
        \begin{equation*}
           = \prs*{\abs*{\sum\limits_{k = 0}^{n - 1} f_k} - \abs*{S_{m + 4} - \overline{S_m + 4}}} - \abs*{\sum\limits_{k = m + 5}^{n - 1}f_k}
        \end{equation*}
        \begin{equation*}
           > \prs*{1 - \frac{6\epsilon}{1 - 6\sqrt\epsilon}}\abs*{\sum\limits_{k = 0}^{n - 1} f_k} - \abs*{\sum\limits_{k = m + 5}^{n - 1}f_k}
        \end{equation*}
        \begin{equation*}
           > \prs*{1 - \frac{6\epsilon}{1 - 6\sqrt\epsilon}}\prs*{1 - 4 \sqrt\epsilon}2^{\exp\prs*{f_m'}} - \epsilon2^{\exp\prs*{f_m'}}
        \end{equation*}
        \begin{equation}
          = \prs*{1 - 4\sqrt\epsilon - \frac{6\epsilon\prs*{1 - 4\sqrt\epsilon}}{1 - 2\sqrt\epsilon} - \epsilon} 2^{\exp\prs*{f_m'}}
        \end{equation}
        which, assuming $\epsilon << 1$, can be simplified to
        \begin{equation}
          \abs*{\overline{S_{m + 4}}} > 2^{\exp\prs*{f_m'} - 1}
        ...
        \begin{equation}
          \abs*{f_k} < 2 \cdot 2^{\exp\prs*{f_k'}} \leq 2 \cdot 2^{\exp\prs*{f_m} - p - 2} = \epsilon \cdot 2^{\exp\prs*{f_m'} - 1}
          \label{eq:maxfmfive}
        \end{equation}
        ...
        \begin{equation*}
          \abs*{S_{n - 1} - \overline{S_{n - 1}}} = \abs*{S_{n - 1} - \overline{S_{m + 4}}} \leq \abs*{S_{m + 4} - \overline{S_{m + 4}}} + \abs*{\sum\limits_{k = m + 5}^{n - 1}f_k}
        \end{equation*}

        \begin{equation*}
          < \frac{6\epsilon}{1 - 6\sqrt\epsilon}\abs*{\sum\limits_{k = 0}^{n - 1}f_k} + \frac{\epsilon}{1 - 4\sqrt\epsilon}\abs*{\sum\limits_{k = 0}^{n - 1} f_k}
        \end{equation*}
        \begin{equation*}
          < \frac{7\epsilon}{1 - 6\sqrt\epsilon}\abs*{\sum\limits_{k = 0}^{n - 1}f_k}
        \end{equation*}
    \end{proof}
    ...
    \begin{equation}
      ...
      \abs*{T - \mathcal{Y}} < n  2^{W  \prs*{1 - K} - 1}\max\abs*{x_k}
    \end{equation}
    ...
    \begin{equation}
      \label{eq:repbound}
      \abs*{T - \mathcal{Y}} < n \cdot \max\prs*{2^{W  \prs*{1 - K} - 1} \max\abs*{x_k}, 2^{e_{\min} - 1}}
    \end{equation}

    By  \eqref{eq:gammadecreases} and \eqref{eq:gammadecreasesfast}, Theorem \ref{thm:mysortsum} applies to yield

    \begin{equation*}
      \abs*{\mathcal{Y} - \overline{\mathcal{Y}}} < \frac{7\epsilon}{1 - 6\sqrt\epsilon}\abs*{\mathcal{Y}}
    \end{equation*}

    By the triangle inequality

    \begin{equation*}
      \abs*{\mathcal{Y}} \leq \abs*{T} + \abs*{T - \mathcal{Y}} < n \cdot \max\prs*{2^{W  \prs*{1 - K} - 1}  \max\abs*{x_k}, 2^{e_{\min} - 1}} + \abs*{T}
    \end{equation*}
    ...
    \begin{equation*}
      \abs*{T - \overline{\mathcal{Y}}} \leq \abs*{T - \mathcal{Y}} + \abs*{\mathcal{Y} - \overline{\mathcal{Y}}}
    \end{equation*}

    \begin{equation*}
      < n \cdot \max\prs*{2^{W  \prs*{1 - K} - 1}  \max\abs*{x_k}, 2^{e_{\min} - 1}} + \frac{7\epsilon}{1 - 6\sqrt\epsilon} \abs*{\mathcal{Y}}
    \end{equation*}

    \begin{equation*}
      < n \cdot \max\prs*{2^{W  \prs*{1 - K} - 1}  \max\abs*{x_k}, 2^{e_{\min} - 1}}
    \end{equation*}
    \begin{equation*}
      + \frac{7\epsilon}{1 - 6\sqrt\epsilon} \prs*{n \cdot \max\prs*{2^{W  \prs*{1 - K} - 1}  \max\abs*{x_k}, 2^{e_{\min} - 1}} + \abs*{T}}
    \end{equation*}

    \begin{equation}
      < \prs*{1 + \frac{7\epsilon}{1 - 6\sqrt\epsilon}} \prs*{n \cdot \max\prs*{2^{W \prs*{1 - K} - 1} \max\abs*{x_k}, 2^{e_{\min} - 1}}} + \frac{7\epsilon}{1 - 6\sqrt\epsilon} \abs*{T}
      \label{eq:error}
    \end{equation}
    ...
    \begin{equation*}
      \abs*{T - \overline{\mathcal{Y}}} < \prs*{1 + \frac{7\epsilon}{1 - 6\sqrt\epsilon}} \prs*{n \cdot \max\prs*{2^{W \prs*{1 - K} - 1} \max\abs*{x_k}, 2^{e_{\min} - 1}}} + \frac{7\epsilon}{1 - 6\sqrt\epsilon}\abs*{T}
    \end{equation*}

    \begin{equation}
    \approx n 2^{W  \prs*{1 - K} - 1} \max\abs*{x_k} + 7  \epsilon \abs*{T}
    ...
    \begin{equation*}
      \abs*{\mathcal{Y}} \leq \abs*{\overline{\mathcal{Y}}} + \abs*{\mathcal{Y} - \overline{\mathcal{Y}}}
    \end{equation*}

    Applying the bound on $|\mathcal{Y} - \overline{\mathcal{Y}}|$ yields

    \begin{equation*}
      \abs*{\mathcal{Y}} < \abs*{\overline{\mathcal{Y}}} + \frac{7\epsilon}{1 - 6\sqrt\epsilon}\abs*{\mathcal{Y}}
    \end{equation*}

    After simplification,

    \begin{equation*}
      \abs*{\mathcal{Y}} < \prs*{\frac{1}{1 - \frac{7\epsilon}{1 - 6\sqrt\epsilon}}}  \abs*{\overline{\mathcal{Y}}}
    \end{equation*}

    \begin{equation*}
      < \frac{1 - 6 \sqrt\epsilon}{1 - 6 \sqrt \epsilon - 7\epsilon}  \abs*{\overline{\mathcal{Y}}}
    \end{equation*}
    ...
    \begin{equation*}
      < n \cdot \max\prs*{2^{W  \prs*{1 - K} - 1}  \max\abs*{x_k}, 2^{e_{\min} - 1}} + \frac{7\epsilon}{1 - 6\sqrt\epsilon}\frac{1 - 6 \sqrt\epsilon}{1 - 6 \sqrt \epsilon - 7\epsilon}\abs*{\overline{\mathcal{Y}}}
    \end{equation*}

    \begin{equation}
      < n \cdot \max\prs*{2^{W  \prs*{1 - K} - 1}  \max\abs*{x_k}, 2^{e_{\min} - 1}} + \frac{7\epsilon}{1 - 6 \sqrt \epsilon - 7\epsilon}\abs*{\overline{\mathcal{Y}}}
      \label{eq:error2}
    \end{equation}
    ...
    \begin{equation*}
      \abs*{T - \overline{\mathcal{Y}}} < n \cdot \max\prs*{2^{W  \prs*{1 - K} - 1}  \max\abs*{x_k}, 2^{e_{\min} - 1}} + \frac{7\epsilon}{1 - 6 \sqrt \epsilon - 7\epsilon}  \abs*{\overline{\mathcal{Y}}}
    \end{equation*}

    \begin{equation}
      \approx n  2^{W  \prs*{1 - K} - 1} \max\abs*{x_k}+ 7  \epsilon  \abs*{\overline{\mathcal{Y}}}
    ...
    \begin{equation*}
    n \cdot \max\prs*{2^{W  \prs*{1 - K} - 1}  \max\abs*{x_k}, 2^{e_{\min} - 1}} + \prs*{\frac{\prs*{2  K - 1}  \epsilon}{1 - \prs*{2  K - 1}  \epsilon}}  \sum\limits_0^{2  K - 1}\abs*{\gamma_k}
    \end{equation*}

    \begin{equation*}
    \leq n \cdot \max\prs*{2^{W  \prs*{1 - K} - 1}  \max\abs*{x_k}, 2^{e_{\min} - 1}} + \prs*{\frac{\prs*{2  K - 1}  \epsilon}{1 - \prs*{2  K - 1}  \epsilon}}  n \cdot \max\abs*{x_k}
    \end{equation*}

    \begin{equation}
    \approx n \cdot \max\abs*{x_k} \prs*{2^{W  \prs*{1 - K} - 1} + \prs*{2  K - 1}  \epsilon}
