...
\section{Notation and Background}
  ...
  Assume that floating point arithmetic complies with the IEEE 754-2008 standard \cite{ieee754} in some ``to nearest'' rounding mode (no specific tie breaking behaviour is required) and that underflow occurs gradually, although methods to handle abrupt underflow will be considered in Section \ref{sec:indexed_implementation_underflow_abrupt}.
  ...
  $\roundtonearestinfty(x, k)$ rounds $x$ to the nearest multiple of $2^k$, breaking ties away from 0. A property of such rounding is shown in  \eqref{eq:round}
...
\section{The Indexed Type}
  ...
  \subsection{Section Summary}
    An indexed type is is composed of several accumulators, each accumulating input in certain ranges (more formally defined later as bins). We begin in Section \ref{sec:indexed_binning} with an original mathematical theory to support the binning scheme. We develop this theory to concisely describe and prove correctness of algorithms throughout the paper (especially Algorithms \ref{alg:depositrestricted} and \ref{alg:deposit}). Section \ref{sec:indexed_representation} serves as a summary of the indexed type defined in \cite{repsum}.
  ...
  \subsection{Binning}
    ...
    We use $i \leq i_{\max} = \floor{(e_{\max} - e_{\min} + p - 1)/W}$ to ensure that $a_i > e_{\min} - p + 1$ as discussed in Section \ref{sec:indexed_implementation_underflow_gradual}. This means that the greatest bin is
    ...
    As discussed in Section \ref{sec:indexed_implementation_overflow}, we must also assume
    \begin{equation}
      2\times W > p + 1
      \label{eq:wlower}
    \end{equation}
    ...
    Finally, it is possible that $d(x, 0)$ may be too large to represent as a floating point number. Overflow of this type is accounted for in Section \ref{sec:indexed_implementation_overflow}.
  ...
  \subsection{Representation}
    ...
    An indexed type $Y$ is a data structure composed of several accumulator data structures $Y_0, ..., Y_{L - 1}$. The number of accumulators in an indexed type is referred to as its \textbf{length} (In \cite{repsum}, a length $L$ indexed type is referred to as a ``$K$-fold'' indexed type). Due to their low accuracy, indexed types of length 1 are not considered.
  ...
  \subsection{Implementation}
    ...
    \subsubsection{Primary and Carry}
      ...
      \begin{equation}
        ...
        \label{eq:pri}
      \end{equation}
      This simplifies the process of extracting the slices of input in bins $(a_{I + j}, b_{I + j}]$. It will be shown in Theorem \ref{thm:ddepositrestricted} that if we represent each primary value ${\mathcal{Y}_j}_P$ as in (\eqref{eq:pri}) and keep ${Y_j}_P$ within the range $(\epsilon^{-1} \times 2^{a_{I + j}}, 2 \times \epsilon^{-1} \times 2^{a_{I + j}})$, then Algorithm \ref{alg:depositrestricted} extracts the slices of $x$ in bins $(a_I, b_I], ..., (a_{I + L - 1}, b_{I + L - 1}]$ and adds them to ${Y_0}_P, ..., {Y_{L - 1}}_P$ without error (and hence reproducibly) for all $x \in \F$, where $|x| < 2^{b_I}$.

      \begin{alg}
        Extract slices of $x \in \F$, where $|x| < 2^{b_I}$, in bins $(a_I, b_I], ..., (a_{I + L - 1}, b_{I + L - 1}]$ and add to indexed type $Y$. Here, $(r | 1)$ represents the result of setting the last bit of the significand ($m_{p - 1}$) of floating point number $r$ to 1. This is a restatement of lines 18-20 of Algorithm $6$ in \cite{repsum}.
        \begin{algorithmic}[1]
          \Require
      ...
      \begin{thm}
        Let $Y$ be an indexed type of index $I$ and length $L$. Assume that we run Algorithm \ref{alg:depositrestricted} on $Y$ and some $x \in \F$, $|x| < 2^{b_I}$. If all requirements of the algorithm are satisfied, then the amount added to ${Y_j}_P$ is exactly $d(x, I + j)$.
        \label{thm:ddepositrestricted}
      \end{thm}
      \begin{proof}
        Throughout the proof, assume that the phrase ``for all $j$'' means ``for all $j \in \{0, ..., L - 1\}$.'' Assume also that $r_j$ and $S_j$ refer to the value of $r$ and $S$ after executing line \ref{alg:depositrestricted:split} in the $j^{th}$ iteration of the loop. Finally, assume ${Y_j}_P$ to refers to the initial value of ${Y_j}_P$ and $S_j$ refers to the final value of ${Y_j}_P$. Therefore, $S_j - {Y_j}_P$ is the amount added to ${Y_j}_P$.
      ...
      Because $d(x, I + j) = 0$ for bins with $x < 2^{a_{I + j}}$, the top $L$ nonzero accumulator values can be computed reproducibly by computing the values in the top $L$ accumulators needed for the largest $x$ seen so far. Upon encountering an $x \geq 2^{b_I}$, the accumulators can then be shifted upwards (towards index $0$) as necessary. \cite{repsum} contains further discussions of the reproducibility of the algorithm.

      In order to keep the primary fields in the necessary range during a deposit operation and to keep the representation of $Y_j$ unique, ${Y_j}_P$ is routinely renormalized to the range $[1.5 \times\epsilon^{-1} \times 2^{a_{I + j}}, 1.75 \times\epsilon^{-1} \times 2^{a_{I + j}})$. Section \ref{sec:basicops_renormalize} contains a discussion of how often such renormalizations are necessary.
      ...
      \begin{equation}
        ...
        \label{eq:acc}
      \end{equation}
      Therefore, using  \eqref{eq:acc}, the value $\mathcal{Y}$ represented by an indexed type $Y$ of index $I$ and length $L$ (the sum of $Y$'s accumulators) is
      \begin{equation}
        \mathcal{Y} = \sum\limits_{j = 0}^{L - 1} \mathcal{Y}_j = \sum\limits_{j = 0}^{L - 1} (({Y_j}_P - 1.5 \times\epsilon^{-1}\times 2^{a_{I + j}}) + {Y_j}_C \times 0.25\times\epsilon^{-1}\times2^{a_{I + j}})
    ...
    \subsubsection{Overflow}
      ...
      \begin{alg}
        Extract components of $x \in \F$, where $|x| < 2^{b_I}$, in bins $(a_I, b_I], ..., (a_{I + L - 1}, b_{I + L - 1}]$ and add to indexed type $Y$ of index $I$. Here, $(r | 1)$ represents the result of setting the last bit of the significand ($m_{p - 1}$) of floating-point $r$ to 1.
        \begin{algorithmic}[1]
          \Require
          ...
          \Function{Deposit}{L, x, Y}
            \If{Y has index 0}
              ...
              \State ${Y_{L - 1}}_P = {Y_{L - 1}}_P + (r | 1)$
            \Else
              \State\Call{DepositRestricted}{L, x, Y} \label{alg:deposit:callrestricted}
            \EndIf
          \EndFunction
          \Ensure
      ...
      \begin{thm}
        Let $Y$ be an indexed type of index $I$ and length $L$. Assume that we run Algorithm \ref{alg:deposit} on $Y$ and some $x \in \F$, $|x| < 2^{b_I}$. If all requirements of the algorithm are satisfied, then the ``Ensure'' claim of Algorithm \ref{alg:deposit} holds.
        \label{thm:ddeposit}
      \end{thm}
      \begin{proof}
    ...
    \subsubsection{Gradual Underflow}
      ...
      \begin{thm}
        For any primary field ${Y_j}_P$ of an indexed type $Y$ of index $I$ and any $r \in \F$ where $|r| \leq 2^{b_{I + j}}$, $\ulp(r) < \frac{1}{2}\ulp({Y_j}_P)$ and $a_{I + j} > e_{\min} - p + 1$.
      ...
      \begin{equation*}
      e_{\min} - p + 2 + ((e_{\max} - e_{\min} + p- 1) \mod W) \geq e_{\min} - p + 2
      \end{equation*}
      $\exp({Y_j}_P) = a_{I + j} + p > 2^{e_{\min} + 1}$ so ${Y_j}_P$ is normalized and therefore $\ulp({Y_j}_P) = 2^{a_{I + j} + 1} > 2^{e_{\min} - p + 2}$.

      If $r$ is normalized, then because $\ulp(r) \leq 2^{1 - p}|r| \leq 2^{b_{I + j} - (p - 1)} = 2^{a_{I + j} + W - (p - 1)}$, and $W < p - 2$, we have $\ulp(r) < \frac{1}{2}\ulp({Y_j}_P)$. (This case is considered in \cite{repsum}).
    ...
    \subsubsection{Exceptions}
      ...
      This behavior follows the behavior for exceptional values in IEEE 754-2008 floating point arithmetic and is therefore easy to implement. At the beginning of Algorithm \ref{alg:deposit}, we may simply check $x$ and ${Y_0}_P$ for the exceptional values \verb|Inf|, \verb|-Inf|, and \verb|NaN|. If any one of $x$ or ${Y_0}_P$ is indeed exceptional, we add $x$ to the (possibly finite) ${Y_0}_P$. Otherwise, we deposit the finite value normally.
...
\section{Basic Operations}
  \subsection{Section Summary}
    Here we reorganize algorithms in \cite{repsum} into a set of basic operations on an indexed type. Two simple original algorithms relating to the index of an indexed type are given in Section \ref{sec:basicops_index}. Theoretical summaries of algorithms (with some improvements) from \cite{repsum} are provided in Sections \ref{sec:basicops_update}, \ref{sec:basicops_deposit}, \ref{sec:basicops_renormalize}, \ref{sec:basicops_reduce}. Section \ref{sec:basicops_convert} provides an original algorithm (with an improved error bound) to obtain the value represented by an indexed type. Sections \ref{sec:basicops_error} and \ref{sec:basicops_limits} extend analysis in \cite{repsum} to the new algorithms.
  \subsection{Index}
    ...
    Note that the floor function is necessary in Algorithm \ref{alg:iindex} to account for the case of ${Y_0}_P$, which has exponent $2^{e_{\max}}$ as discussed in Section \ref{sec:indexed_implementation_overflow}.
  ...
  \subsection{Renormalize}
    ...
    When depositing values into an indexed type $Y$ of index $I$ and length $L$, the assumption is made that ${Y_j}_P \in (\epsilon^{-1}\times 2^{a_{I + j}}, 2 \times \epsilon^{-1}\times 2^{a_{I + j}})$ throughout the routine. To enforce this condition, the indexed type must be renormalized every $2^{p - w - 2}$ deposit operations, as will be shown later in this section. The renormalization procedure is shown in Algorithm \ref{alg:renorm}.
    \begin{alg}
      For an indexed type $Y$ of index $I$ and length $L$, assuming ${Y_j}_P \in [1.25 \times \epsilon^{-1}\times 2^{a_{I + j}}, 2 \times \epsilon^{-1}\times 2^{a_{I + j}})$, renormalize $Y$ such that ${Y_j}_P \in [1.5 \times \epsilon^{-1}\times 2^{a_{I + j}}, 1.75 \times \epsilon^{-1}\times 2^{a_{I + j}})$.
      \begin{algorithmic}[1]
        \Require
    ...
    and if $n \leq 2^{p - W - 2}$, then after the $n^{th}$ deposit
  ...
  \subsection{Update}
    ...
    As noted in Algorithm \ref{alg:deposit}, when adding $x \in \F$ to an indexed type $Y$ of index $I$ and length $L$, we make the assumption that $|x| < 2^{b_I}$. However, this is not always the case and sometimes it is necessary to adjust the index of $Y$. This adjustment is called an \textbf{update}. The process of updating $Y$ to the necessary index is summarized succinctly in Algorithm \ref{alg:update}.
    \begin{alg}
      Update indexed type $Y$ of index $I$ and length $L$ to have an index suitable to deposit $x$.
      \begin{algorithmic}[1]
        \Require
          \Statex ${Y_0}_P \in (\epsilon^{-1} \times 2^{a_{I}}, 2 \times \epsilon^{-1} \times 2^{a_I})$
        \Function{Update}{L, x, Y}
          \State \Call{IIndex}{Y, I}
          \State \Call{Index}{x, J}
          \If{$J < I$}
            \State $[{Y_{\min(I - J, L)}}_P, ..., {Y_{L - 1}}_P] = [{Y_0}_P, ..., {Y_{L - 1 - \min(I - J, L)}}_P]$
            \State $[{Y_0}_P, ..., {Y_{\min(I - J, L) - 1}}_P] = [1.5 \times \epsilon^{-1} \times a_{J}, ..., 1.5 \times \epsilon^{-1} \times a_{\min(I, L + J) - 1}]$
            \State $[{Y_{\min(I - J, L)}}_C, ..., {Y_{L - 1}}_C] = [{Y_0}_C, ..., {Y_{L - 1 - \min(I - J, L)}}_C]$
            \State $[{Y_0}_C, ..., {Y_{\min(I - J, L) - 1}}_C] = [0, ..., 0]$
          \EndIf
        \EndFunction
        \Ensure
          \Statex The bin $(a_{J}, b_{J}]$ satisfies $2^{b_{J}} > |x| \geq 2^{a_{J}}$.
    ...
    It should be noted that if ${Y_0}_P$ is 0, then the update is performed as if $I + L < J$. If ${Y_0}_P$ is \verb|Inf|, \verb|-Inf|, or \verb|NaN|, then $Y$ is not modified by an update.
  ...
  \subsection{Reduce}
    ...
    \begin{alg}
      For an indexed type $Y$ of index $I$ and length $L$ and indexed type $Z$ of index $J$ and length $L$, add $Z$ to $Y$.
      \begin{algorithmic}[1]
        \Ensure
        ...
        \Function{Reduce}{L, Y, Z}
          \State \Call{IIndex}{Y, I}
          \State \Call{IIndex}{Z, J}
          \If{$J < I$}
            \State $R = Z$
            \State \Call{Reduce}{L, $R$, $Y$}
            \State $Y = R$
          \EndIf
          \State $j = J - I$
          \While{$j < L$}
  ...
  \subsection{Convert}
    ...
    \begin{alg}
      Convert floating point $x$ to an indexed type $Y$ with length $L$.
      \begin{algorithmic}[1]
        \Function{ConvertFloatToIndexed}{L, x, Y}
          \State $Y = 0$
          \State \Call{Update}{L, x, Y}
          \State \Call{Deposit}{L, x, Y} \label{alg:conv2indexed:deposit}
          \State \Call{Renorm}{L, Y}
        \EndFunction
        \Ensure
    ...
    \begin{equation*}
      n \times \max|x_k|\times (2^{W \times (1 - L) - 1} + (2 \times L - 1) \times \epsilon)
    \end{equation*}
    ...
    \begin{equation*}
      n \times \max|x_k|\times 2^{W \times (1 - L) - 1} + 15 \epsilon |\sum\limits_{k = 0}^{n - 1} x_k|
    \end{equation*}
    ...
    Consider an indexed type $Y$ of index $I$ and length $L$.
    ...
    \begin{equation*}
    > \exp({\mathcal{Y}'_{L - 2}}_C)
    \end{equation*}

    \begin{equation*}
    > \exp({\mathcal{Y}'_{L - 3}}_P)
    \end{equation*}

    \begin{equation*}
    > \exp({\mathcal{Y}'_{L - 1}}_C)
    \end{equation*}

    \begin{equation*}
    > \exp({\mathcal{Y}'_{L - 2}}_P)
    \end{equation*}

    \begin{equation*}
    > \exp({\mathcal{Y}'_{L - 1}}_P)
    \end{equation*}

    These unnormalized floating point numbers may, for convenience of notation, be referred to in decreasing order of unnormalized exponent as $\gamma'_0, ..., \gamma'_{2 \times L - 1}$.
    ...
    \begin{equation}
      \exp(\gamma_0') > ... > \exp(\gamma_{2 \times L - 1}')
      \label{eq:gammadecreases}
    \end{equation}

    It should be noted that if $\gamma_k$ is a primary field, then either $\gamma_{k + 1}$ or $\gamma_{k + 2}$ is a primary field. If $\gamma_k$ is a carry field, then either $\gamma_{k + 1}$ or $\gamma_{k + 2}$ is a carry field (with the exception of $\gamma_{2 \times L - 3}$, but in this case we have $\exp(\gamma_{2 \times L - 3}) = a_{I + L - 1} + p + p - 3 \geq a_{I + L - 1} + p + \ceil{\frac{p + 1}{2}} - 1 = \exp(\gamma_{2 \times L - 1}) + \ceil{\frac{p + 1}{2}}$). Therefore, as $2 \times W > p + 1$, for all $k \in \{0, ..., 2 \times L - 1\}$
    \begin{equation}
    ...
    Notice that $|\gamma'_0| = |{\mathcal{Y}'_0}_C| < 2 \times 2^{\exp({\mathcal{Y}'_0}_C)} = 2 \times 2^{e_{\max} + 1 - W + p + p - 3}$ and $\exp(\gamma'_0) > ... > \exp(\gamma'_{2 \times L - 1})$.  Therefore $|\gamma'_k| \leq 2^{e_{\max} - W + p + p - 1 - k}$. The absolute value represented by an indexed type can therefore be bounded by

    \begin{equation}
      \label{eq:maxindexedvalue}
      \sum\limits_{k = 0}^{2 \times L - 1} |\gamma_k| < \sum\limits_{k = 0}^{2 \times L - 1} 2^{e_{\max} - W + p + p - 1 - k} < \sum\limits_{k = 0}^{\infty} 2^{e_{\max} - W + p + p - 1 - k} = 2^{e_{\max} - W + p + p}
    \end{equation}
    ...
    \begin{alg}
      Convert indexed type $Y$ of index $I$ and length $L$ to floating point $x$. Here, $z$ is a floating point type with at least the original precision and maximum exponent $E_{\max}$ greater than $e_{\max} - W + p + p$
      \begin{algorithmic}[1]
        \Function{ConvertIndexedToFloat}{L, x, Y}
          \State $z = {\mathcal{Y}_0}_C$
          \For{$j = 1 \To L - 1$}
            \State $z = z + {\mathcal{Y}_j}_C$
            \State $z = z + {\mathcal{Y}_{j - 1}}_P$
          \EndFor
          \State $z = {\mathcal{Y}_{L - 1}}_P$
  ...
  When $\exp(\gamma'_k) < e_{\max} - \rho - 1$, the sum may be scaled back up and the remaining numbers added without scaling. Notice that no overflow can occur during addition in this algorithm. If an overflow is to occur, it will happen only when scaling back up. As the fields in the indexed type are reproducible, such an overflow condition is reproducible.
    ...
    \begin{alg}
      Convert an indexed type $Y$ of index $I$ and length $L$ to floating point $x$. Here, $z$ is a floating point number with precision $\rho > p$
      \begin{algorithmic}[1]
        \Function{ConvertIndexedToFloat}{L, x, Y}
          \State $j = 1$
          \While{$j \leq 2 \times L$ and $\exp(\gamma_j) \geq e_{\max} - \rho - 1$}
            \State $z = z + (\gamma_j / 2^{p + p - W - 2})$
            \State $j = j + 1$
          \EndWhile
          \State $z = z \times 2^{p + p - W - 2}$
          \While{$j \leq 2 \times L$}
            \State $z = z + \gamma_j$
            \State $j = j + 1$
          \EndWhile
          \State $x = z$
  ...
  \subsection{Error Bound}
    ...
    \begin{proof}
      ...
      If $n - 1 \leq m + 4$, we may combine  \eqref{eq:smrelsbound} and \eqref{eq:smsrelsbound} with Equation 2.4 from \cite{higham} to obtain
 ...
 \leq (10 \times (\frac{1}{(1 - 5 \epsilon)(1 - 4 \sqrt\epsilon)}) + 4 \times (\frac{1 + 4 \sqrt \epsilon}{(1 - 4 \epsilon)(1 - 4 \sqrt\epsilon)}))\times \epsilon \times|\sum\limits_{k = 0}^{n - 1}f_k|
      ...
      If $n - 1 > m + 4$, we may combine  \eqref{eq:smrelsbound} and \eqref{eq:smsrelsbound} with Equation 2.4 from \cite{higham} to obtain
 ...
 \leq ((\frac{5\epsilon}{1 - 5 \epsilon})(\frac{2}{1 - 4 \times \sqrt\epsilon}) + (\frac{4\epsilon}{1 - 4 \epsilon})(\frac{1 + 4 \times \sqrt\epsilon}{1 - 4 \times \sqrt\epsilon}))|\sum\limits_{k = 0}^{n - 1}f_k|
      ...
      By  \eqref{eq:sbound}, \eqref{eq:smfiveerror}, and \eqref{eq:boundfmfivesum}.
        ...
        \begin{equation}
          ...
          \label{eq:minsmfour}
        \end{equation}
        As we may assume $\epsilon << 1$,  \eqref{eq:minsmfour} can be simplified to
        \begin{equation}
          |\overline{S_{m + 4}}| > 2^{\exp(f_m') - 1}
          \label{eq:minsmfoursimple}
        \end{equation}

        Using  \eqref{eq:fmfiveexp},
    ...
    Consider the indexed sum $Y$ of length $L$ of floating point numbers $x_0, ..., x_{n - 1}$. We denote the true sum $\sum \limits_{k = 0}^{n - 1} x_k$ by $T$, the true value of the indexed sum as obtained using  \eqref{eq:indexedvalue} by $\mathcal{Y}$, and the floating point approximation of $\mathcal{Y}$ obtained using an appropriate algorithm from section \ref{sec:basicops_convert} by $\overline{\mathcal{Y}}$.
    ...
    \begin{equation}
      ...
      |T - \mathcal{Y}| < n \times 2^{W \times (1 - L) - 1} \times \max|x_k|
    \end{equation}
    ...
    \begin{equation}
      \label{eq:repbound}
      |T - \mathcal{Y}| < n \times \max(2^{W \times (1 - L) - 1} \times \max|x_k|, 2^{e_{\min} - 1})
    \end{equation}
    ...
    \begin{equation*}
      |\mathcal{Y}| \leq |T| + |T - \mathcal{Y}| < n \times \max(2^{W \times (1 - L) - 1} \times \max|x_k|, 2^{e_{\min} - 1}) + |T|
    \end{equation*}
    ...
    \begin{equation*}
      < n \times \max(2^{W \times (1 - L) - 1} \times \max|x_k|, 2^{e_{\min} - 1}) + g(\epsilon)\times \epsilon\times |\mathcal{Y}|
    \end{equation*}

    \begin{equation*}
      < n \times \max(2^{W \times (1 - L) - 1} \times \max|x_k|, 2^{e_{\min} - 1})
    \end{equation*}
    \begin{equation*}
      + g(\epsilon)\times \epsilon\times (n \times \max(2^{W \times (1 - L) - 1} \times \max|x_k|, 2^{e_{\min} - 1}) + |T|)
    \end{equation*}

    \begin{equation}
      < (1 + g(\epsilon)\times\epsilon) \times n \times \max(2^{W \times (1 - L) - 1} \times \max|x_k|, 2^{e_{\min} - 1}) + g(\epsilon)\times\epsilon\times |T|
      \label{eq:error}
    \end{equation}
    ...
    \begin{equation*}
      |T - \overline{\mathcal{Y}}| < (1 + g(\epsilon)\times\epsilon) \times n \times \max(2^{W \times (1 - L) - 1} \times \max|x_k|, 2^{e_{\min} - 1}) + g(\epsilon)\times\epsilon\times|T|
    \end{equation*}

    \begin{equation}
    \approx n \times \max|x_k|\times 2^{W \times (1 - L) - 1} + 15 \times \epsilon \times|T|
    ...
    \begin{equation}
      < n \times \max(2^{W \times (1 - L) - 1} \times \max|x_k|, 2^{e_{\min} - 1}) + (\frac{g(\epsilon)\times\epsilon}{1 - g(\epsilon)\times\epsilon}) \times |\overline{\mathcal{Y}}|
      \label{eq:error2}
    \end{equation}
    ...
    \begin{equation*}
      |T - \overline{\mathcal{Y}}| < n \times \max(2^{W \times (1 - L) - 1} \times \max|x_k|, 2^{e_{\min} - 1}) + (\frac{g(\epsilon)\times\epsilon}{1 - g(\epsilon)\times\epsilon}) \times |\overline{\mathcal{Y}}|
    \end{equation*}

    \begin{equation}
      \approx n \times \max|x_k|\times 2^{W \times (1 - L) - 1} + 15 \times \epsilon \times |\overline{\mathcal{Y}}|
    ...
    \begin{equation*}
    n \times \max(2^{W \times (1 - L) - 1} \times \max|x_k|, 2^{e_{\min} - 1}) + (\frac{(2 \times L - 1) \times \epsilon}{1 - (2 \times L - 1) \times \epsilon}) \times \sum\limits_0^{2 \times L - 1}|\gamma_k|
    \end{equation*}

    \begin{equation*}
    \leq n \times \max(2^{W \times (1 - L) - 1} \times \max|x_k|, 2^{e_{\min} - 1}) + (\frac{(2 \times L - 1) \times \epsilon}{1 - (2 \times L - 1) \times \epsilon}) \times n \times \max|x_k|
    \end{equation*}

    \begin{equation}
    \approx n \times \max|x_k|\times (2^{W \times (1 - L) - 1} + (2 \times L - 1) \times \epsilon)
    \label{eq:baderrorapprox}
    \end{equation}

    Which is not as tight a bound as \eqref{eq:errorapprox}, and grows linearly as the user increases the length in an attempt to increase accuracy.

  \subsection{Limits}
    \label{sec:basicops_limits}
    As discussed previously, the minimum length accepted by ReproBLAS is 2. The maximum useful length is $\floor{(e_{\max} - e_{\min} + p - 1)/W}$, as this covers all of the bins.
