...
\section{Basic Operations}
  ...
  \subsection{Convert}
    ...
    As the maximum absolute value of the sum is bounded by $2^{e_{\max} - W + p + p}$, a sufficient scaling factor is $2^{p + p - W}$, bounding the maximum absolute value of the scaled sum by $2^{e_{\max}}$. When $\exp(y'_k) < e_{\max} - P$, we may scale the sum back up and add the remaining numbers without scaling.
