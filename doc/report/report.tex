...
\section{The Indexed Type}
  ...
  \subsection{Implementation}
    ...
    \end{samepage}
      ...
      Because $d(x, I + j) = 0$ for bins with $x < 2^{a_{I + j}}$, the top $K$ nonzero accumulator values can be computed reproducibly by computing the values in the top $K$ accumulators needed for the largest $x$ seen so far. Upon encountering an $x \geq 2^{b_I}$, the accumulators can then be shifted upwards (towards index $0$) as necessary. \cite{repsum} contains further discussions of the reproducibility of the algorithm.
    ...
    \end{samepage}
      ...
      \end{equation*}
      we have $\exp({Y_j}_P) = a_{I + j} + p > 2^{e_{\min} + 1}$ so ${Y_j}_P$ is normalized and therefore $\ulp({Y_j}_P) = 2^{a_{I + j} + 1} > 2^{e_{\min} - p + 2}$.
    ...
    \subsubsection{Exceptions}
      ...
      Note that an explicit check for exceptional values is necessary at some point in the summation, as Algorithm \ref{alg:deposit} cannot correctly sum an \verb|Inf| or \verb|-Inf| (it changes the \verb|Inf| to a \verb|NaN| when setting the last bit of $r$). Although checking for exceptional values explicitly is expensive, the cost can be reduced if several values are to be summed in the same method (for example, using the methods discussed in Section \ref{sec:idxdBLAS}).
...
\section{Basic Operations}
  ...
  \subsection{Index}
    ...
    Note that the floor function is necessary in Algorithm \ref{alg:iindex} to account for the case of $Y$ with index $0$, which has $\exp({Y_0}_P) = 2^{e_{\max}}$ as discussed in Section \ref{sec:indexed_implementation_overflow}.
  ...
  \subsection{Update}
    ...
    It should be noted that if ${Y_0}_P$ is 0, then the update is performed as if $I + K < J$. If ${Y_0}_P$ is \verb|Inf|, \verb|-Inf|, or \verb|NaN|, then $Y$ is not modified by an update.
  ...
  \subsection{Convert}
    ...
    \begin{equation*}
      n \times \max|x_k|\times 2^{W \times (1 - K) - 1} + 15 \epsilon |\sum\limits_{k = 0}^{n - 1} x_k|
    \end{equation*}
    ...
    It is possible that future implementors may make modifications to the indexed type (adding multiple carry fields, changing the binning scheme, etc.) such that the summation of its fields cannot be reordered to satisfy the assumptions of Theorem \ref{thm:mysortsum}. In such an event, methods discussed in $\cite{sortsum}$ provide a reasonably efficient yet more general way to sum the fields while still maintaining accuracy.
  ...
  \subsection{Error Bound}
    ...
    \begin{thm}
      ...
      Where
      \begin{equation}
        g(\epsilon) = (10 \times (\frac{1}{1 - 5 \epsilon}) + 4 \times (\frac{1 + 4 \sqrt\epsilon}{1 - 4 \epsilon}) + 1) \times (\frac{1}{1 - 4 \sqrt\epsilon}) \approx 15
        \label{eq:g}
      \end{equation}
    \end{thm}
    \end{samepage}
    ...
    \begin{proof}
      ...
      We now show $|S_m| \geq 2 \times 2^{\exp(f_m')}$. Assume for contradiction that $|S_m| < 2 \times 2^{\exp(f_m')}$. Because $S_m \in \ulp(f_m')\Z$, this would imply that $S_m$ is representable as a floating point number, a contradiction as $\overline{S_m} \neq S_m$. Therefore, we have
      \begin{equation}
      ...
      Let $T_0 = \overline{T_0} = S_{m - 1}$, $T_k = T_{k - 1} + f_{k + m - 1}$, and $\overline{T_k} = \fl(\overline{T_{k - 1}} + f_{k + m - 1})$ so that $T_{n - m} = \sum \limits_{k = 0}^{n - 1} f_k$.

      Notice that since $S_{m - 1} = \overline{S_{m - 1}}$,
      \begin{equation*}
        |S_{n - 1} - \overline{S_{n - 1}}| = |T_{n - m} - \overline{T_{n - m}}|
      \end{equation*}

      We may combine  \eqref{eq:smrelsbound} and \eqref{eq:smsrelsbound} with Equation 2.4 from \cite{higham} to obtain

      \begin{equation*}
        |S_{\min(n - 1, m + 4)} - \overline{S_{\min(n - 1, m + 4)}}| = |T_{\min(n - m, 5)} - \overline{T_{\min(n - m, 5)}}|
      \end{equation*}
      \begin{equation*}
        \leq (\frac{5\epsilon}{1 - 5 \epsilon})|S_{m - 1} + f_m| + (\frac{4\epsilon}{1 - 4 \epsilon})\sum\limits_{k = m + 1}^{\min(n - 1, m + 4)}|f_k|
      \end{equation*}
      \begin{equation*}
        = (\frac{5\epsilon}{1 - 5 \epsilon})|S_m| + (\frac{4\epsilon}{1 - 4 \epsilon})\sum\limits_{k = m + 1}^{\min(n - 1, m + 4)}|f_k|
      \end{equation*}
      \begin{equation*}
      ...
      \end{equation*}
      \begin{equation}
        \leq (10 \times (\frac{1}{1 - 5 \epsilon}) + 4 \times (\frac{1 + 4 \sqrt\epsilon}{1 - 4 \epsilon})) \times (\frac{1}{1 - 4 \sqrt\epsilon})\times \epsilon \times|\sum\limits_{k = 0}^{n - 1}f_k|
        \label{eq:smfiveerror}
      \end{equation}

      If $n - 1 \leq m + 4$, \eqref{eq:smfiveerror} implies
      \begin{equation*}
        |S_{n - 1} - \overline{S_{n - 1}}| \leq (10 \times (\frac{1}{1 - 5 \epsilon}) + 4 \times (\frac{1 + 4 \sqrt\epsilon}{1 - 4 \epsilon})) \times (\frac{1}{1 - 4 \sqrt\epsilon})\times \epsilon \times|\sum\limits_{k = 0}^{n - 1}f_k|
      \end{equation*}
      \begin{equation*}
      ...
      By  \eqref{eq:sbound}, \eqref{eq:smfiveerror}, and \eqref{eq:boundfmfivesum}.
        ...
        \end{equation*}
        \begin{equation*}
           \geq (1 - (10 \times (\frac{1}{1 - 5 \epsilon}) + 4 \times (\frac{1 + 4 \sqrt\epsilon}{1 - 4 \epsilon})) \times (\frac{1}{1 - 4 \sqrt\epsilon}) \times \epsilon)\times |\sum\limits_{k = 0}^{n - 1} f_k| - \sum\limits_{k = m + 5}^{n - 1}|f_k|
        \end{equation*}
        \begin{equation*}
           \geq (1 - (10 \times (\frac{1}{1 - 5 \epsilon}) + 4 \times (\frac{1 + 4 \sqrt\epsilon}{1 - 4 \epsilon})) \times (\frac{1}{1 - 4 \sqrt\epsilon}) \times \epsilon)\times(1 - 4 \sqrt\epsilon)\times 2^{\exp(f_m')} - \epsilon \times 2^{\exp(f_m')}
        \end{equation*}
        \begin{equation}
          = (1 - 4 \times \sqrt\epsilon - 10\epsilon \times (\frac{1}{1 - 5 \epsilon}) - 4\epsilon \times (\frac{1 + 4 \sqrt\epsilon}{1 - 4 \epsilon}) - \epsilon )\times 2^{\exp(f_m')}
          \label{eq:minsmfour}
        \end{equation}
        ...
        \begin{equation*}
          < (10 \times (\frac{1}{1 - 5 \epsilon}) + 4 \times (\frac{1 + 4 \sqrt\epsilon}{1 - 4 \epsilon})) \times (\frac{1}{1 - 4 \sqrt\epsilon})\times \epsilon \times|\sum\limits_{k = 0}^{n - 1}f_k| + (\frac{\epsilon}{1 - 4 \times \sqrt\epsilon})|\sum\limits_{k = 0}^{n - 1} f_k|
        \end{equation*}
        \begin{equation*}
          = (10 \times (\frac{1}{1 - 5 \epsilon}) + 4 \times (\frac{1 + 4 \sqrt\epsilon}{1 - 4 \epsilon}) + 1) \times (\frac{1}{1 - 4 \sqrt\epsilon})\times \epsilon \times|\sum\limits_{k = 0}^{n - 1}f_k|
        \end{equation*}
        \begin{equation*}
          = g(\epsilon)\times \epsilon\times |\sum\limits_{k = 0}^{n - 1}f_k|
        \end{equation*}
    ...
    \begin{equation}
    \approx n \times \max|x_k|\times 2^{W \times (1 - K) - 1} + 15 \times \epsilon \times|T|
    ...
    \begin{equation}
      \approx n \times \max|x_k|\times 2^{W \times (1 - K) - 1} + 15 \times \epsilon \times |\overline{\mathcal{Y}}|
