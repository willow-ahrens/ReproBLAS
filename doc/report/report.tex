...
\section{The Indexed Type}
  ...
  \subsection{Implementation}
    ...
    \subsubsection{Primary and Carry}
      ...
      The value ${\mathcal{Y}_j}_S$ is stored in the primary field ${Y_j}_S$ as an offset from $1.5\times \epsilon^{-1} \times 2^{a_{i + j}}$. This greatly simplifies the process of extracting the components of the input belonging in bin $(a_i, b_i]$.
      ...
      \begin{alg}
        ...
        \begin{algorithmic}
          \Function{Deposit}{fold, x, Y}
            \State $r \gets x$
            \For{$j = 0 \to (fold - 2)$}
              \State $S \gets {Y_j}_S + (r | 1)$
              \State $q \gets S - {Y_j}_S$
              \State ${Y_j}_S \gets S$
              \State $r \gets r - q$
            \EndFor
      ...
      \begin{alg}
        ...
        \begin{algorithmic}
          \Function{Deposit0}{fold, x, Y}
            ...
            \State $S \gets {Y_0}_S + (r | 1)$
            \State $q \gets S - {Y_0}_S$
            \State ${Y_0}_S \gets S$
            \State $q \gets q \times 2^{p - W + 1}$
            \State $r \gets x - q$
            \For{$i = 1 \to (fold - 2)$}
              \State $S \gets {Y_i}_S + (r | 1)$
              \State $q \gets S - {Y_i}_S$
              \State ${Y_i}_S \gets S$
              \State $r \gets r - q$
            \EndFor
            \State ${Y_{fold - 1}}_S \gets {Y_{fold - 1}}_S + (r | 1)$
...
\section{Basic Operations}
  \subsection{Update}
    ...
    \begin{alg}
      ...
      \begin{algorithmic}
        \Function{Update}{fold, x, Y}
          \If{$i' < i$}
            \State $[{Y_{\min(i - i', fold)}}_S, ..., {Y_{fold - 1}}_S] \gets [{Y_0}_S, ..., {Y_{fold - 1 - \min(i - i', fold)}}_S]$
            \State $[{Y_0}_S, ..., {Y_{\min(i - i', fold) - 1}}_S] \gets [1.5 \times \epsilon^{-1} \times a_{i'}, ..., 1.5 \times \epsilon^{-1} \times a_{\min(i, fold + i') - 1}]$
            \State $[{Y_{\min(i - i', fold)}}_C, ..., {Y_{fold - 1}}_C] \gets [{Y_0}_C, ..., {Y_{fold - 1 - \min(i - i', fold)}}_C]$
            \State $[{Y_0}_C, ..., {Y_{\min(i - i', fold) - 1}}_C] \gets [0, ..., 0]$
          \EndIf
        \EndFunction
      \end{algorithmic}
      \label{alg:update}
    ...
    It should be noted that if ${Y_0}_S$ is 0, then the update is performed as if $i + fold < i'$. If ${Y_0}_S$ is \verb|Inf|, \verb|-Inf|, or \verb|NaN|, then $Y$ is not modified by an update.
  ...
  \subsection{Reduce}
    ...
    \begin{alg}
      For an indexed type $Y$ (the indexed sum of some $x_0, ..., x_{n - 1} \in \F$) with index $i$ and indexed type $Z$ with index $t$ (the indexed sum of some $x_n, ..., x_{n + m - 1} \in \F$), add $Z$ to $Y$. That is, set $Y$ to the indexed sum of $x_0, ..., x_{n + m - 1}$. At the start and end of the algorithm, we may assume

${Y_j}_S \in [1.5 \times \epsilon^{-1}\times 2^{a_{i + j}}, 1.75 \times \epsilon^{-1}\times 2^{a_{i + j}})$ and

${Z_j}_S \in [1.5 \times \epsilon^{-1}\times 2^{a_{t + j}}, 1.75 \times \epsilon^{-1}\times 2^{a_{t + j}}).$
      \begin{algorithmic}
        \Function{Reduce}{fold, Y, Z}
          \If{$t > i$}
            \State $T \gets Z$
            \State \Call{Reduce}{fold, $T$, $Y$}
            \State $Y \gets T$
          \EndIf
          \For{$j \in 0 \to t$}
            \State ${Y_{j + i - t}}_S \gets {Y_{j + i - t}}_S + {Z_j}_S - 1.5 \times \epsilon^{-1} \times 2^{a_{i + j}}$
            \State ${Y_{j + i - t}}_C \gets {Y_{j + i - t}}_C + {Z_j}_C$
          \EndFor
      ...
      \begin{alg}
        ...
        \begin{algorithmic}
          \Function{DepositVectorized}{fold, $\vec{x}$, Y}
            ...
            \State $ \gets Y_P + \sum(\vec{Z_P} - \langle Y_P, ..., Y_P \rangle)$
          \EndFunction
        \end{algorithmic}
