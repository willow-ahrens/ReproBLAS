    ...
    \subsubsection{Overflow}
      ...
      \begin{proof}
        ...
        If $|x| < 2^{a_0} = 2^{e_{\max} + 1 - W}$, then we have $|r| < 2^{a_0 - p + W - 1}$ in line \ref{alg:deposit:scaler} regardless of whether or not there is underflow in the division as we scale by a power of two.
         Therefore, since $|r| < 2^{a_0 - p + W - 1}$, ${Y_0}_P - S = 0 = d(x, 0)/2^{p - W - 1}$ by Theorem \ref{thm:dzero}.
        ...
        At this point, all that remains to be shown is that $r = x - d(x, 0)$ in line \ref{alg:deposit:formragain}. In line \ref{alg:deposit:formq}, we know that $q = {Y_0}_P - S = d(x, 0)/2^{p - W + 1}$. By Theorem \ref{eq:dbound}, $d(x, 0) \leq 2 \times 2^{e_{\max}}$ and therefore there is no overflow in line \ref{alg:deposit:scaleq} and as we scale by a power of two, $q = d(x, 0)/2$ in line \ref{alg:deposit:scaleq}. Again we divide into two cases based on the size of $x$.
       make it more clear above that q does not overflow.
        ...
        If $|x| \geq 2^{a_0}$, we have $d(x, 0) = \roundtonearestinfty(x, a_0 + 1)$ by Theorem \ref{thm:dmiddle}, $|x - d(x, 0)| \leq 2^{a_0}$ by Theorem \ref{thm:dround}, and $d(x, 0) \leq 2^{b_0}$ by Theorem \ref{thm:dbound}.  Therefore, we have $|x| \geq |x - d(x, 0)|$ which implies $|x| \geq |x - d(x, 0)/2| \geq |x - d(x, 0)|$. As $|x - d(x, 0)/2| $
        Show that all quantities are in ulp(x)Z and ur done
      \end{proof}

      Algorithm \ref{alg:deposit} has a few key features worth pointing out in the case that $Y$ has index 0. First, since $|x| < 2^{e_{\max} + 1}$, then the value of $r$ after its first assignment satisfies $|r| < 2^{e_{\max} - p + W + 1}$. This shows both that the extraction of the top denormalized bin will work correctly and that the top bin is the greatest bin necessary to deposit any finite $x \in \F$.
      Furthermore, since $x$ is scaled by a power of two, the significand of $r$ is equal to that of $x$.
      Therefore, the amount added to ${Y_0}_P$ is $\roundtonearestinfty(x/2^{p - W + 1}, a_0 - (p - W + 1) + 1) = d(x, 0)/2^{p - W + 1}$. We can therefore scale this slice by $2^{p - W}$ to obtain half the value of the slice of $x$ belonging to bin $0$. As $x < 2 \times 2^{e_{\max}}$, $d(x, 0) \leq 2 \times 2^{e_{\max}}$. It is for this reason that $x$ is halved (to ensure that no overflow occurs) in line \ref{alg:deposit:halfx} before half of $r$ is formed in line \ref{alg:deposit:formr}. Because we know that the unscaled value of $r$ is representable, we can double the half of $r$ in line \ref{alg:deposit:formrtwice}. We can then continue as in Algorithm \ref{alg:depositrestricted}.
...
\section{Basic Operations}
  ...
  \subsection{Renormalize}
    ...
    First, note that $|d(x_k, I + j)| \leq 2^{b_{I + j}}$, where $d(x_k, I + j)$ is the amount added to ${Y_j}_P$ on iteration $j$. This is due to the definition of $d(x_k, a_{I+j}, b_{I + j})$ or to Corollary 3 of \cite{repsum}.
