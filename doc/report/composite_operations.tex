\section{Composite Operations}
  ...
  %We make here an important distinction with respect to parallel reproducibility. If the summation algorithm needs only to be reproducible with respect to If the only source of nonreproducibility in a computation is the reduction tree shape, then only the reduction step must be reproducible for the entire computation to be reproducible. To obtain a general completely reproducible algorithm for summation that is, one must design for reproducibility under both data permutation and reduction tree shape.
