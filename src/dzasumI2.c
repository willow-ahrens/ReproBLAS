...
#include <emmintrin.h>
  ...
  void dzasumI2(int n, double complex* v, int incv, int fold, double complex* sum){
    ...
    double complex tmp[2] __attribute__((aligned(32)));
    SET_DAZ_FLAG;
    switch(fold){
      case 3:{
        int i;
      ...
      default:{
        ...
        for(j = 0; j < fold; j += 1){
          ...
          _mm256_store_pd((double*)tmp, s_buffer[(j * 4)]);
          sum[j] = tmp[0] + tmp[1];
        }
        RESET_DAZ_FLAG
        return;
