...
/**
 * @brief  Get an MPI_DATATYPE representing indexed single precision.
 *
 * Creates (if it has not already been created) and returns a datatype handle
 * for an MPI datatype that represents a indexed single precision type.
 *
...
MPI_Datatype idxdMPI_FLOAT_INDEXED(const int fold){
  ...
  if(!types_initialized[fold]){
    rc = MPI_Type_contiguous(idxd_sinum(fold), MPI_FLOAT, types + fold);
    if(rc != MPI_SUCCESS){
      if (rc == MPI_ERR_TYPE) {
