...
/**
 * @brief  Get an MPI_DATATYPE representing indexed complex double precision.
 *
 * Creates (if it has not already been created) and returns a datatype handle
 * for an MPI datatype that represents an indexed complex double precision type.
 *
...
MPI_Datatype idxdMPI_DOUBLE_COMPLEX_INDEXED(const int fold){
  ...
  if(!types_initialized[fold]){
    rc = MPI_Type_contiguous(idxd_zinum(fold), MPI_DOUBLE, types + fold);
    if(rc != MPI_SUCCESS){
      if (rc == MPI_ERR_TYPE) {
