...
/**
 * @brief  Get an MPI_DATATYPE representing indexed complex single precision.
 *
 * Creates (if it has not already been created) and returns a datatype handle
 * for an MPI datatype that represents a indexed complex single precision type.
 *
...
MPI_Datatype idxdMPI_FLOAT_COMPLEX_INDEXED(const int fold){
  ...
  if(!types_initialized[fold]){
    rc = MPI_Type_contiguous(idxd_cinum(fold), MPI_FLOAT, types + fold);
    if(rc != MPI_SUCCESS){
      if (rc == MPI_ERR_TYPE) {
