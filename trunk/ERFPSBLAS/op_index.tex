  \subsection{Index}
    ...
    Note also that the function $\exp()$ used above is assumed to return the biased exponent of ${Y_0}_P$, so that \textproc{IIndex} returns 0 when ${Y_0}_P$ is \texttt{Inf}, \texttt{-Inf}, or \texttt{NaN} and $i_{\max}$ when ${Y_0}_P$ is 0 or denormalized.
    ...
    Note again that the function $\exp()$ used above is assumed to return the biased exponent of $x$, so that \textproc{Index} returns 0 when $x$ is \texttt{Inf}, \texttt{-Inf}, or \texttt{NaN} and $i_{\max}$ when $x < 2^{a_{i_{\max}}}$.
