...
const char* bench_vecvec_fill_name(int argc, char** argv){
  ...
  snprintf(name_buffer, MAX_LINE * sizeof(char), "Benchmark [scasum]");
  return name_buffer;
}
...
int bench_vecvec_fill_test(int argc, char** argv, int N, int FillX, double ScaleX, double CondX, int incX, int FillY, double ScaleY, double CondY, int incY, int trials){
  ...
  metric_load_long_long("s_add", (long long)2 * N);
  metric_load_long_long("s_orb", (long long)2 * N);
  metric_dump();
