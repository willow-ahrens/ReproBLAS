...
static void bench_rssum_options_initialize(void){
...
int bench_vecvec_fill_show_help(void){
  bench_rssum_options_initialize();
...
const char* bench_vecvec_fill_name(int argc, char** argv){
  ...
  snprintf(name_buffer, MAX_LINE * sizeof(char), "Benchmark [rssum] (fold = %d)", fold._int.value);
  return name_buffer;
}
...
int bench_vecvec_fill_test(int argc, char** argv, int N, int FillX, double RealScaleX, double ImagScaleX, int incX, int FillY, double RealScaleY, double ImagScaleY, int incY, int trials){
  ...
  if(fold._int.value == 0){
    metric_load_double("s_add", (double)(3 * MAX_FOLD * (MAX_FOLD - 1) * 0.5 - 2 * MAX_FOLD) * N);
    metric_load_double("s_orb", (double)MAX_FOLD * (MAX_FOLD - 1) * 0.5 * N);
  }else{
