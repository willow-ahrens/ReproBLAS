...
static void bench_cbcbadd_options_initialize(void){
  ...
  preN._int.header.help       = "cbcbadd preN before cbcbadd";
...
int bench_vecvec_fill_show_help(void){
  bench_cbcbadd_options_initialize();
...
const char* bench_vecvec_fill_name(int argc, char** argv){
  ...
  snprintf(name_buffer, MAX_LINE * sizeof(char), "Benchmark [cbcbadd] (fold = %d)", fold._int.value);
  return name_buffer;
}
...
int bench_vecvec_fill_test(int argc, char** argv, int N, int FillX, double RealScaleX, double ImagScaleX, int incX, int FillY, double RealScaleY, double ImagScaleY, int incY, int trials){
  ...
  X = (float_complex_binned*)util_cvec_alloc(N * binned_cbnum(fold._int.value), 1);
  memset(X, 0, N * binned_cbsize(fold._int.value));
  for(i = 0; i < N; i++){
    binnedBLAS_cbcsum(fold._int.value, preN._int.value, preX + i * preN._int.value * incX, incX, X + i * binned_cbnum(fold._int.value));
  }
  ...
  for(i = 0; i < trials; i++){
    ...
    for(k = 0; k < N; k++){
      binned_cbcbadd(fold._int.value, X + k * binned_cbnum(fold._int.value), ires);
    }
