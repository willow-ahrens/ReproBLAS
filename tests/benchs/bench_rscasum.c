...
static void bench_rscasum_options_initialize(void){
...
int bench_vecvec_fill_show_help(void){
  bench_rscasum_options_initialize();
...
const char* bench_vecvec_fill_name(int argc, char** argv){
  ...
  snprintf(name_buffer, MAX_LINE * sizeof(char), "Benchmark [rscasum] (fold = %d)", fold._int.value);
  return name_buffer;
}
...
int bench_vecvec_fill_test(int argc, char** argv, int N, int FillX, double RealScaleX, double ImagScaleX, int incX, int FillY, double RealScaleY, double ImagScaleY, int incY, int trials){
  ...
  if(fold._int.value == DEFAULT_FOLD){
    ...
    for(i = 0; i < trials; i++){
      sicasum(fold._int.value, N, X, incX, ires);
    }
  ...
  if(fold._int.value == 0){
    ...
    metric_load_double("s_orb", (double)(MAX_FOLD * (MAX_FOLD - 1) * 0.5 + MAX_FOLD) * 2 * N);
  }else{
    metric_load_double("s_add", (double)(3 * fold._int.value - 2) * 2 * N);
    metric_load_double("s_orb", (double)(fold._int.value + 1) * 2 * N);
  }
  metric_dump();
