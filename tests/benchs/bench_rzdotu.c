...
int bench_vecvec_fill_test(int argc, char** argv, int N, int FillX, double RealScaleX, double ImagScaleX, int incX, int FillY, double RealScaleY, double ImagScaleY, int incY, int trials){
  ...
  if(fold._int.value == DEFAULT_FOLD){
    ...
    for(j = 1; j <= MAX_FOLD; j++){
      ...
      zisetzero(j, ires);
      for(i = 0; i < trials; i++){
        zizdotu(j, N, X, incX, Y, incY, ires);
      }
    ...
    for(i = 0; i < trials; i++){
      zizdotu(fold._int.value, N, X, incX, Y, incY, ires);
    }
  ...
  if(fold._int.value == 0){
    metric_load_double("d_add", (double)(3 * MAX_FOLD * (MAX_FOLD - 1) * 0.5 - 2 * MAX_FOLD) * 4 * N);
    metric_load_double("d_orb", (double)MAX_FOLD * (MAX_FOLD - 1) * 0.5 * 4 * N);
  }else{
