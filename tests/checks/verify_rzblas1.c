...
int verify_rzblas1_reproducibility(int N, double complex* X, int incX, double complex* Y, int incY, int func, double complex ref, double_complex_binned *Iref, int max_num_blocks) {
  // GENERATE DATA
  int i;
  double complex res;
  double_complex_binned *Ires = binned_zballoc(DIDEFAULTFOLD);
  int num_blocks = 1;
  ...
  while (num_blocks < N && num_blocks <= max_num_blocks) {
    ...
    if (res != ref) {
      ...
      binned_zbprint(DIDEFAULTFOLD, Iref);
      printf("\nRes I_double_Complex:\n");
      binned_zbprint(DIDEFAULTFOLD, Ires);
      printf("\n");
      return 1;
    }
    num_blocks *= 2;
...
int vecvec_fill_test(int argc, char** argv, int N, int FillX, double RealScaleX, double ImagScaleX, int incX, int FillY, double RealScaleY, double ImagScaleY, int incY){
  int rc = 0;
  double complex ref;
  double_complex_binned *Iref = binned_zballoc(DIDEFAULTFOLD);
  ...
  binned_zbsetzero(DIDEFAULTFOLD, Iref);
