...
static void verify_dbdssq_options_initialize(void){
...
int verify_dbdssq_reproducibility(int fold, int N, double* X, int incX, double refscl, double refssq, double_binned *iref, int max_num_blocks) {
  // GENERATE DATA
  int i;
  double resscl = 0.0;
  double resssq;
...
int vecvec_fill_show_help(void){
  verify_dbdssq_options_initialize();
...
const char* vecvec_fill_name(int argc, char** argv){
  ...
  snprintf(name_buffer, MAX_LINE * sizeof(char), "Verify dbdssq reproducibility fold=%d", fold._int.value);
  return name_buffer;
}
...
int vecvec_fill_test(int argc, char** argv, int N, int FillX, double RealScaleX, double ImagScaleX, int incX, int FillY, double RealScaleY, double ImagScaleY, int incY){
  ...
  refscl = binnedBLAS_dbdssq(fold._int.value, N, X, incX, refscl, iref);
  ...
  rc = verify_dbdssq_reproducibility(fold._int.value, N, X, incX, refscl, refssq, iref, max_num_blocks);
  if(rc != 0){
    return rc;
  }
