...
static void verify_sbcssq_options_initialize(void){
...
int verify_sbcssq_reproducibility(int fold, int N, float complex* X, int incX, float refscl, float refssq, float_binned *iref, int max_num_blocks) {
  // GENERATE DATA
  int i;
  float resscl = 0.0;
  float resssq;
  ...
  while (num_blocks < N && num_blocks <= max_num_blocks) {
    ...
    if (num_blocks == 1)
      resscl = binnedBLAS_sbcssq(fold, N, X, incX, resscl, ires);
    else {
      block_N = (N + num_blocks - 1) / num_blocks;
      for (i = 0; i < N; i += block_N) {
        block_N = block_N < N - i ? block_N : (N-i);
        resscl = binnedBLAS_sbcssq(fold, block_N, X + i * incX, incX, resscl, ires);
      }
    }
    resssq = binned_ssbconv(fold, ires);
    if (resssq != refssq || resscl != refscl) {
      printf("binnedBLAS_sbcssq(X)[num_blocks=%d,block_N=%d] = %g * (%g) != %g * (%g)\n", num_blocks, block_N, resscl, resssq, refscl, refssq);
...
int vecvec_fill_show_help(void){
  verify_sbcssq_options_initialize();
...
const char* vecvec_fill_name(int argc, char** argv){
  ...
  snprintf(name_buffer, MAX_LINE * sizeof(char), "Verify sbcssq reproducibility fold=%d", fold._int.value);
  return name_buffer;
}
...
int vecvec_fill_test(int argc, char** argv, int N, int FillX, double RealScaleX, double ImagScaleX, int incX, int FillY, double RealScaleY, double ImagScaleY, int incY){
  ...
  refscl = binnedBLAS_sbcssq(fold._int.value, N, X, incX, refscl, iref);
  refssq = binned_ssbconv(fold._int.value, iref);
  ...
  rc = verify_sbcssq_reproducibility(fold._int.value, N, X, incX, refscl, refssq, iref, max_num_blocks);
  if(rc != 0){
    return rc;
  }
