...
static void verify_dbzssq_options_initialize(void){
...
int verify_dbzssq_reproducibility(int fold, int N, double complex* X, int incX, double refscl, double refssq, double_binned *iref, int max_num_blocks) {
  // GENERATE DATA
  int i;
  double resscl = 0.0;
  double resssq;
  ...
  while (num_blocks < N && num_blocks <= max_num_blocks) {
    ...
    if (num_blocks == 1)
      resscl = binnedBLAS_dbzssq(fold, N, X, incX, resscl, ires);
    else {
      block_N = (N + num_blocks - 1) / num_blocks;
      for (i = 0; i < N; i += block_N) {
        block_N = block_N < N - i ? block_N : (N-i);
        resscl = binnedBLAS_dbzssq(fold, block_N, X + i * incX, incX, resscl, ires);
      }
    }
    resssq = binned_ddbconv(fold, ires);
    if (resssq != refssq || resscl != refscl) {
      printf("binnedBLAS_dbzssq(X)[num_blocks=%d,block_N=%d] = %g * (%g) != %g * (%g)\n", num_blocks, block_N, resscl, resssq, refscl, refssq);
...
int vecvec_fill_show_help(void){
  verify_dbzssq_options_initialize();
...
const char* vecvec_fill_name(int argc, char** argv){
  ...
  snprintf(name_buffer, MAX_LINE * sizeof(char), "Verify dbzssq reproducibility fold=%d", fold._int.value);
  return name_buffer;
}
...
int vecvec_fill_test(int argc, char** argv, int N, int FillX, double RealScaleX, double ImagScaleX, int incX, int FillY, double RealScaleY, double ImagScaleY, int incY){
  ...
  refscl = binnedBLAS_dbzssq(fold._int.value, N, X, incX, refscl, iref);
  refssq = binned_ddbconv(fold._int.value, iref);
  ...
  rc = verify_dbzssq_reproducibility(fold._int.value, N, X, incX, refscl, refssq, iref, max_num_blocks);
  if(rc != 0){
    return rc;
  }
